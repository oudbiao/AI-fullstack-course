...
""".strip(),
    ...
    {
        "filename": "intro-ai-assistant-repo-evidence-cabinet-en.png",
        "size": "1024x1792",
        "quality": "high",
        "title": "AI Learning Assistant repository evidence cabinet",
        "suggested_page": "docs/intro/ai-learning-assistant-template.md",
        "alt": "AI Learning Assistant repository evidence cabinet: src code, data materials, evals checks, logs traces, docs showcase, and tests regression flow into a portfolio demo.",
        "prompt": """
A vertical English course infographic, workflow-like and lightly comic, explaining that the ai-learning-assistant repository is a portfolio evidence cabinet.
Center of the image: a clear file cabinet or folder tree labeled "ai-learning-assistant repo". From it, draw six layered branches/cards:
src/ code, data/ materials, evals/ checks, logs/ traces, docs/ showcase, tests/ regression.
Show arrows from those six evidence layers into a final bottom card labeled "portfolio demo".
Emphasize the idea: "Evidence flow, not decoration". Use it as a large readable tagline, not tiny text.
Keep visible text sparse with short English labels only. Make labels large and clean. Avoid dense paragraphs, small fake file names, gibberish, watermarks, and real brand logos.
Style: professional beginner-friendly technical course illustration, vertical 9:16, clear folder icons, structured layers, simple arrows, enough whitespace.
""".strip(),
    },
    {
        "filename": "intro-ai-assistant-repo-evidence-cabinet.png",
        "size": "1024x1792",
        "quality": "high",
        "title": "AI 学习助手仓库证据柜图",
        "suggested_page": "docs/intro/ai-learning-assistant-template.md",
        "alt": "AI 学习助手仓库证据柜图：src 代码、data 材料、evals 检查、logs 轨迹、docs 展示、tests 回归测试汇入 portfolio demo。",
        "prompt": """
一张竖版中文课程图解，主题是“ai-learning-assistant repository 是作品集证据柜”。
画面中心是一个清晰的文件柜或文件夹树，标签为“ai-learning-assistant 仓库”。从中心分层连接 6 个证据层：
src/ 代码、data/ 材料、evals/ 检查、logs/ 轨迹、docs/ 展示、tests/ 回归。
用箭头把这 6 层证据最后汇入底部卡片：“portfolio demo”。
重点表达一句大字短句：“不是装饰目录，而是证据流”。这句话要清晰可读，不要做成小字。
文字只用自然中文短标签，src/、data/、evals/、logs/、docs/、tests/、portfolio demo 可以保留英文。不要密集段落、不要小字乱码、不要真实品牌 logo、不要水印。
风格专业、温和、新手友好，竖版 9:16，文件夹图标清楚，分层结构清晰，箭头简单，留白充足。
""".strip(),
    },
    {
        "filename": "intro-ai-assistant-repo-evidence-cabinet-ja.png",
        "size": "1024x1792",
        "quality": "high",
        "title": "AI 学習アシスタント リポジトリ証拠キャビネット図",
        "suggested_page": "docs/intro/ai-learning-assistant-template.md",
        "alt": "AI 学習アシスタントのリポジトリ証拠キャビネット図：src コード、data 教材、evals チェック、logs トレース、docs ショーケース、tests 回帰テストが portfolio demo へ流れる。",
        "prompt": """
AI 学習アシスタントの repository が「ポートフォリオの証拠キャビネット」であることを説明する、日本語の縦長コース図解。
中央に分かりやすいファイルキャビネットまたはフォルダツリーを置き、ラベルは「ai-learning-assistant repo」。そこから 6 つの証拠レイヤーへ分岐：
src/ コード、data/ 教材、evals/ チェック、logs/ トレース、docs/ ショーケース、tests/ 回帰。
6 つのレイヤーから下部の最終カード「portfolio demo」へ矢印で流れ込む構図にする。
大きく読みやすい短いメッセージとして「飾りの目次ではなく、証拠の流れ」と入れる。小さな注釈にしない。
文字は自然な日本語の短いラベルだけにし、src/、data/、evals/、logs/、docs/、tests/、portfolio demo などは英語表記でよい。細かすぎる文字、文字化け、透かし、実在ブランドロゴは禁止。
スタイルはプロフェッショナルでやさしい初心者向け教材図解、縦 9:16、明快なフォルダアイコン、分かりやすい階層、簡単な矢印、十分な余白。
""".strip(),
    },
    {
