...
DIRECT_TRIPLET_GROUPS: list[dict[str, Any]] = [
    ...
    },
    {
        ...
        "scene": "A tiny 1D GAN lab result board. Do not draw long code blocks. Show real samples as a bell-shaped 1D distribution centered near 2.0. Show generator fake distributions across steps with exact printed values: step 001 fake_mean=0.025 fake_std=0.117, step 100 fake_mean=1.093 fake_std=0.204, step 200 fake_mean=2.988 fake_std=0.291, step 300 fake_mean=1.384 fake_std=0.056. Show a two-player update loop: train D with real and detached fake, then train G so D says real. Mark fake_std=0.056 as a low-diversity warning, not as the best final line. Mention loss_d/loss_g are diagnostics but samples and diversity must be inspected.",
        ...
        "shared_layout": "Vertical 9:16. Top title and subtitle. Upper section shows the D and G two-step adversarial loop with detach on fake during the D step. Middle section has a timeline of four fake distributions compared with the real center at 2.0, using exact mean/std values. Bottom has a warning card for low diversity and a checklist: inspect samples, mean, std, and losses. Same layout, colors, values, and reading path across zh/en/ja.",
        "variants": {
            "zh": {
    ...
    },
    {
        ...
        "scene": "A tiny 2D VAE lab result board. Do not draw long code blocks. Show two input clusters centered near [1,0] and [-1,0]. Show VAE flow: encoder -> mu/logvar -> reparameterize z = mu + eps*std -> decoder. Show training result cards with exact numbers: epoch 001 recon=0.5903 kl=0.0293 loss=0.5917; epoch 050 recon=0.0335 kl=0.9007 loss=0.0785; epoch 100 recon=0.0261 kl=0.8229 loss=0.0673; epoch 200 recon=0.0244 kl=0.7138 loss=0.0601. Show generated points from random z: [1.075,-0.014], [-0.997,-0.001], [-1.118,-0.054], [0.553,0.041], [0.740,0.021]. Bottom highlights that recon drops, KL stays controlled, and generated points are decoded from random z, not copied.",
        ...
        "shared_layout": "Vertical 9:16. Top title and subtitle. Upper section shows two 2D input clusters and VAE flow with mu/logvar and z sampling. Middle section has recon/KL/loss cards over epochs with exact values. Lower section shows generated 2D points plotted near the two clusters plus a note that random z is decoded, not copied. Bottom shows the reconstruction versus KL tradeoff. Same layout, colors, values, and reading path across zh/en/ja.",
        "variants": {
            "zh": {
