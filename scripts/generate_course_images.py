...
DIRECT_TRIPLET_GROUPS: list[dict[str, Any]] = [
    ...
    },
    {
        ...
        "scene": "A decision tree lab result board based on the Iris and diabetes script. Do not invent extra curve numbers, node counts, sample counts, or MAE values. Use only these numeric results from the lesson: max_depth=1 has train=0.670 and test=0.658, max_depth=2 has train=0.964 and test=0.947, max_depth=3 has train=0.982 and test=0.974, max_depth=None has train=0.982 and test=0.974. Show depth 1, depth 3, and a too-complex tree as visual tree sketches above a simple trend chart; labels can say too simple, balanced, too complex. For pruning, use exactly: ccp_alpha=0.0000 test=0.921 leaves=7; ccp_alpha=0.0067 test=0.921 leaves=5; ccp_alpha=0.2636 test=0.658 leaves=2. For regression tree, use exactly: max_depth=2 MAE=47.3 leaves=4; max_depth=4 MAE=44.4 leaves=14; max_depth=None MAE=48.7 leaves=25. Include a step-like prediction sketch but no made-up MAE chart values. The teaching point is that depth and pruning control complexity; more leaves do not automatically improve test performance.",
        ...
        "shared_layout": "Vertical 9:16. Top title and subtitle. Upper half has three tree thumbnails labeled max_depth=1, max_depth=3, and too complex, plus a small four-row score card for depth 1/2/3/None using the exact train/test values from the scene. Middle has a pruning slider from low alpha to high alpha with exactly three cards: 0.0000 -> 7 leaves / test 0.921, 0.0067 -> 5 leaves / test 0.921, 0.2636 -> 2 leaves / test 0.658. Lower section has a regression tree step prediction sketch plus a three-row MAE card with exact values 47.3, 44.4, 48.7. Keep the same tree sizes, alpha positions, scores, colors, and reading path across zh/en/ja. Do not draw extra numeric curves.",
        "variants": {
            "zh": {
                "title": "决策树实验结果怎么读",
    ...
    },
    {
        ...
        "scene": "A clustering lab result visualization. Show a blob dataset on the left with K candidates 2, 3, 4, 5. An elbow/silhouette board marks K=3 as best: inertia drops sharply and silhouette peaks at 0.869. Show three centroid markers on the blob scatter. On the right, show a two-moons dataset where K-Means cuts the moons incorrectly with ARI 0.475, while DBSCAN with eps=0.25 follows the curved moons with ARI 0.995 and only one noise point. Lower corner shows hierarchical clustering on blobs with ARI 1.0 as a compact-cluster alternative.",
        ...
        "shared_layout": "Vertical 9:16. Top title and subtitle. Left half has round blob scatter, K-selection mini chart, and K=3 badge. Right half has moon scatter comparison: K-Means wrong cut versus DBSCAN curve-following result. Bottom has a small method-choice strip: K-Means for round groups, DBSCAN for curved/noisy groups, hierarchical for inspection. Keep chart shapes, colors, metric numbers, and panel order identical across zh/en/ja.",
        "variants": {
            "zh": {
                "title": "聚类结果不只看数字",
