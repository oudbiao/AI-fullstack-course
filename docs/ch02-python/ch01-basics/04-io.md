...
1. `input()` returns strings. Convert age with `int()` and distance with `float()` before doing arithmetic.
2. The self-introduction output should have top and bottom separators and should reflect the values typed by the user.
3. The receipt should align columns and show subtotals `119.80`, `46.50`, and total `166.30` for the given data.
4. For `km = 5`, miles are about `3.11` and meters are `5000.00`.
