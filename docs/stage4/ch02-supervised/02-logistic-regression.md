...
```mermaid
flowchart LR
    A["先定义分类问题"] --> B["为什么线性回归不够"]
    B --> C["用 Sigmoid 把输出变成概率"]
    C --> D["再定义交叉熵损失"]
    D --> E["最后再看决策边界和多分类扩展"]
```
...
### 4.1 新人第一次做多分类时怎么理解最稳？

一个更稳的顺序通常是：

1. 先理解二分类版逻辑回归
2. 再把多分类理解成“一组二分类器”或 Softmax 概率分布
3. 最后再回到 sklearn 看 `multinomial` 这些实现细节
