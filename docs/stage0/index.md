...
本阶段的小项目很简单：创建一个名为 `ai-learning-lab` 的学习仓库，里面包含一个 Python 环境说明、一个能运行的 `hello_ai.py`、一个 Git 提交历史，以及一个记录学习命令的笔记文件。

这个项目的价值不是功能复杂，而是让你第一次完整走通“创建项目、写代码、运行代码、提交代码、记录过程”的开发闭环。