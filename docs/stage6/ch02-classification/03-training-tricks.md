...
### 3.2 再看一个最小“训练日志 -> 技巧建议”示例

```python
logs = {
    "train_loss": [1.2, 0.9, 0.7, 0.5],
    "val_loss": [1.1, 1.0, 1.05, 1.2],
    "gpu_memory_gb": 10.5,
}


def suggest_fix(logs):
    if logs["val_loss"][-1] > logs["val_loss"][1]:
        return "先考虑正则化、数据增强或更早停。"
    if logs["gpu_memory_gb"] > 10:
        return "先考虑混合精度或减小 batch size。"
    return "先保持 baseline，别急着加太多技巧。"


print(suggest_fix(logs))
```

这个例子虽然简单，但它更接近真实训练时的思维方式：
