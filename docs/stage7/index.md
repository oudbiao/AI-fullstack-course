...
这里把第五阶段设为前置，不是为了把路线拉长，而是因为：

- 第五阶段已经给了你神经网络训练、序列建模和最基本的深度学习直觉
- 第七阶段开始把这些能力放进文本表示、文本任务和预训练范式里

如果你的目标是后面进入第八 A 阶段，那么这一阶段会明显降低理解大模型原理时的跳跃感。
...
- 第四章序列标注，更适合在你已经先分清句级任务和词级任务后再系统补
- 第五章序列到序列，更适合在你已经先看顺编码器、解码器和注意力这条线后再深入
- 第七章项目实践，更适合在你已经分清分类、抽取、生成三类任务差别后再做

**先做哪个最小项目 / 闭环最划算**

先做一个最小文本分类项目，把 `预处理 -> 表示 -> 模型 -> 评估 -> 错例分析` 跑通。第一次先把“文本怎样表示、任务怎样闭环”建立起来，比一上来就追 BERT、GPT 或复杂生成任务更划算。
