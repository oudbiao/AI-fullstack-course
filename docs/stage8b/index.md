...
这里把第八 A 设为前置，不是为了多加一道门槛，而是因为：

- 第八 A 更偏模型原理、Prompt、微调和对齐这些“能力从哪里来、怎样被约束”的底座
- 第八 B 更偏把这些能力真正接到知识、接口、产品和工程上

如果跳过第八 A 直接做第八 B，你通常也能把 Demo 跑起来，但会更难判断问题到底出在模型、提示、检索还是系统层。
...
1. 先把第一章 `RAG 主线` 看顺
   先知道文档、切分、检索、重排和评估怎样连起来。
2. 再看第二章 `模型部署`
   先知道模型怎样被调用、怎样暴露统一入口。
3. 然后看第三章 `应用开发`
   先把会话、函数调用和结构化输出接进最小应用。

**哪些内容可以第二遍再补**

- 第四章工程化里的 `asyncio`、并发、监控和容器部署，更适合在你已经跑通最小系统后系统补
- 如果你暂时没有 `asyncio` 基础，可以先完成前三章，再回头补选修模块 B 的并发编程后进入第四章
- 各类复杂框架封装和更重的生产部署方案，也更适合作为二刷补强

**先做哪个最小项目 / 闭环最划算**

先做一个最小 RAG 应用：文档入库、检索召回、模型回答，再加一个简单会话或结构化输出接口。先把“知识 -> 调用 -> 应用”跑通，再回头补日志、并发和部署，收获通常最高。

## 如果你想按更完整的顺序系统推进

如果你已经先跑通前面这条最小主线，更完整的节奏通常是：
