...
| 时间 | 做什么 | 能说出这句话就停 |
|---|---|---|
| 20 分钟 | 阅读部署闭环 | “模型只有在运行时、API、日志、评估和回滚都清楚后，才算进入部署。” |
| 20 分钟 | 运行环境检查 | “我知道这台机器能不能用 CUDA，还是只能用 CPU。” |
| 25 分钟 | 运行 runbook 脚本 | “我能根据硬件和项目约束选择运行时。” |
| 25 分钟 | 建一个五条 Prompt 评估表 | “我能在改运行时或微调前比较模型行为。” |
| 30 分钟 | 写适配决策 | “我能解释为什么选 Prompt、RAG、量化、LoRA 或不微调。” |
