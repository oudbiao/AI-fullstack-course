...
开源大模型最容易让人分心的是模型名字和榜单分数。但工程上真正先决定成败的，通常是这些约束：

| 约束 | 如果没想清楚会怎样 |
|---|---|
| 许可证 | 模型能跑，但不能用于你的分发或商业场景 |
| 硬件 | 下载后显存不够，或者服务延迟不可接受 |
| 隐私 | 私有文档、日志或 prompt 进入了不该进入的环境 |
| 运行时 | 模型能单次推理，但无法稳定 API 服务化 |
| 评估集 | 一次回答看起来不错，但换问题就不可比较 |
| 停止/回滚 | GPU 一直烧钱，或者坏版本无法快速切回 |

所以本章的学习顺序故意是：先选计算路线和证据包，再选模型和运行时。模型越大，越需要先把这些边界写清楚。
...
| 字段 | 要记录什么 |
|---|---|
| 候选模型 | 精确模型名，必要时写清 revision |
| 模型来源 | Hub、镜像、本地路径或内部仓库 |
| 许可证 | 许可证和必须遵守的使用限制 |
| 参数量和量化方式 | 模型尺寸，以及是否使用量化格式 |
| 上下文长度 | 本次任务准备依赖的上下文窗口 |
| 语言/领域能力 | 为什么适合当前语言、任务或领域 |
| 预计显存或内存 | 当前 runtime 下需要的内存 |
| 预计磁盘 | 下载和缓存需要预留的空间 |
| 运行时 | Transformers、llama.cpp、Ollama、vLLM、SGLang 或其他 runtime |
| 选择理由 | 用证据说明为什么第一轮选它就够 |
| 暂不选择的模型 | 看过但没有选的模型 |
| 风险 | 许可证、隐私、下载、显存、速度和输出质量风险 |
