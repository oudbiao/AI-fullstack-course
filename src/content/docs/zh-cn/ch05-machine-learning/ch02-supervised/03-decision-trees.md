...
线性回归和逻辑回归很稳，也容易解释系数，但它们默认在学一条直线、一个平面或一个线性分数。真实表格数据里，很多规则更像这样：

```text
如果年龄较大且余额较低，则风险高；
如果职业是学生且消费频率高，则可能是另一类。
```

这类“分段规则”和“特征交互”用线性模型表达起来不够直观。决策树补上的能力是：

| 线性模型的限制 | 决策树的回答 |
|---|---|
| 边界偏线性 | 通过多次切分形成非线性区域 |
| 交互项要人工构造 | 树路径天然表达条件组合 |
| 系数解释对业务同学不直观 | 输出 if-else 风格规则路径 |
| 分类和回归要分开理解 | CART 用同一套树思想处理两类任务 |

代价也很明显：单棵树容易把训练集细节记得太死，所以后面会自然走向随机森林和 Boosting。
