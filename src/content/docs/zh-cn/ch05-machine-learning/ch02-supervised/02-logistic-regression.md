...
线性回归擅长预测连续数值，例如价格、销量、温度。但分类任务要回答的是“属于哪一类”，直接让线性回归预测 `0` 或 `1` 会有几个麻烦：

| 线性回归做分类的问题 | 逻辑回归的回答 |
|---|---|
| 输出可能小于 0 或大于 1 | 用 sigmoid / softmax 压成概率 |
| 预测值不好解释成类别置信度 | 输出 `predict_proba()`，再用阈值决策 |
| 错得很自信和错得不确定惩罚不一样 | 用交叉熵更适合概率分类 |
| 业务常需要调误报和漏报 | 保留概率，让阈值成为产品决策 |

所以逻辑回归不是“低级分类器”，而是把线性模型从连续预测推进到概率分类。后面更复杂的模型，也常会保留“概率 + 阈值 + 错误成本”这条思路。
