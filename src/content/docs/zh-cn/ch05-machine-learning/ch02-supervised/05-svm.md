...
逻辑回归会学一条分类边界，决策树会学一组规则。但还有一个很自然的问题：

```text
如果很多边界都能分开训练样本，哪一条更稳？
```

SVM 的回答是：选离最近样本更远的边界，也就是最大间隔。它补上的不是“又一个分类 API”，而是一种泛化直觉：

| 旧问题 | SVM 的回答 |
|---|---|
| 只要训练集分对，边界可能贴得太近 | 最大化 margin，给新样本留空间 |
| 线性边界不够处理弯曲数据 | 用 kernel 构造非线性边界 |
| 样本靠近边界时最容易出问题 | 重点关注 support vectors |

这也是为什么 SVM 很适合教学：它把“分类边界要稳”讲得很清楚。真实项目中，它要和逻辑回归、树集成一起比较，而不是默认胜出。
