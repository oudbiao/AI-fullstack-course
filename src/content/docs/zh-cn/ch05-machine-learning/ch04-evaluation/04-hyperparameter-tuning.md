...
调参不是模型效果不好时的第一反应。更合理的顺序是：

```text
先确认指标 -> 再确认验证方式 -> 再看偏差/方差 -> 最后才搜索超参数
```

如果特征没有信号、标签有问题、数据泄漏或指标选错了，调参只会把问题藏得更深。超参数调优真正适合解决的是：模型方向基本对了，但某些训练前设置会影响泛化、速度或成本。
...
1. 先切出 final holdout。
2. 用 CV 搜索出一组参数。
3. 看 final holdout 后觉得不够好。
4. 回去改 grid，再看 final holdout。
5. 重复几次后，final holdout 也被你“训练”进决策里了。
```

到第 5 步时，它已经不再是最终测试，而是另一个验证集。安全做法是：final holdout 只用来做一次最终检查；如果看完后还要继续改方案，就需要重新留出新的最终检查数据，或明确记录这次结果已经不能当作最终泛化估计。
