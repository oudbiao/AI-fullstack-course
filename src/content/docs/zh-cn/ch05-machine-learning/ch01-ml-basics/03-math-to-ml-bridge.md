...
![数学到机器学习训练地图](/img/course/ch05-math-to-ml-training-map.webp)
...
:::note[拆开符号]
`X @ w` 在 Python 里表示矩阵乘法。`@` 不是机器学习的神秘语法，而是把特征矩阵和权重向量相乘的运算符。`b` 是截距，可以理解成每个预测都会加上的基础值。
:::
### 线性代数在第 5 站后面会出现在哪？
...
![梯度下降四步训练循环](/img/course/ch05-gradient-descent-four-step-loop.webp)
...
![机器学习数学训练循环漫画](/img/course/ch05-math-training-loop-comic.webp)
