...
边缘部署的复盘要从设备约束开始，而不是从模型名字开始。设备内存、功耗、散热、离线要求和日志能力，都会决定模型是否真的能长期运行。

最小证据包括：冷启动输出、连续运行记录、离线行为、内存变化和失败时的恢复方式。能跑一次 demo 不等于能部署到边缘设备。

交付检查时，把“设备能跑”拆成三种状态：能启动、能持续运行、能在异常后恢复。三者缺一不可。尤其是离线场景，要提前写清网络断开时保留哪些功能、丢弃哪些功能，以及用户会看到什么提示。
