...
复盘这个页面时，把它当成状态机，而不是漂亮截图。每次用户动作都要写清起始状态、可见反馈、最终状态，以及失败后的恢复路径。

这个习惯能让 AI 界面更可靠。模型调用可能慢、空、部分返回或出错，所以 UI 要显示进度、避免静默失败，并给用户下一步动作。最小证据是空输入、成功和错误三条路径的截图或记录。

接入真实后端前，先把假状态版本做完整。这样你会先确定文案、布局、加载提示、错误恢复和重试契约。后端到来时，你只是替换数据来源，而不是同时发明整套用户体验。

交付检查时，记录三条路径：空输入、成功返回、失败重试。每条路径都要有用户能看懂的状态提示。只要其中一条路径让用户不知道发生了什么，这个 AI 前端就还没有闭环。
