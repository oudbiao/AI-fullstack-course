...
如果文档永远清晰、单行、正放，那么 OCR 可以看起来像“识别图片里的字”。真实文档会复杂得多：文字可能分栏、倾斜、被表格隔开，字段还要按业务含义组织。

| 旧问题 | 拆分出的阶段 |
|---|---|
| 不知道图上哪些区域是文字 | 文本检测先找文字框 |
| 找到了区域但不知道写了什么 | 文本识别把图像块转成字符序列 |
| 字都读出来但顺序乱、字段不清 | 版面理解恢复阅读顺序和结构 |
| 金额、日期、编号错一位就影响业务 | 字段级校验和置信度检查 |

所以 OCR 的历史拆分不是为了复杂，而是因为“字在哪”“字是什么”“这段字属于什么结构”是三类不同错误源。
