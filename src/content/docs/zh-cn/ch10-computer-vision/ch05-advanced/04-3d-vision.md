...
### 3D 视觉为什么不能只靠普通 2D 图像

2D 图像把三维世界压成一个平面。压平之后，很多空间信息会丢失：远近、真实尺寸、遮挡关系、相机视角都可能变得模糊。3D 视觉就是为了解决这些空间问题。

| 2D 图像的限制 | 3D 视觉补上的信息 |
|---|---|
| 同样大小的物体可能只是远近不同 | depth 估计距离 |
| 图像框只说明平面位置 | 点云提供空间坐标 |
| 单张图很难判断真实结构 | 多视角几何利用不同视角恢复结构 |
| 物体朝向和空间关系不清 | 3D box、姿态和重建描述空间状态 |

所以 3D 视觉不是“图片多一个维度”这么简单，而是把视觉任务从平面识别推进到空间测量和几何推理。
