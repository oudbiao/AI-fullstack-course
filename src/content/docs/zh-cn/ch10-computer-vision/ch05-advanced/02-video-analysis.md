...
最早做视频任务时，一个朴素办法是把视频抽成很多帧，再把每帧当图片处理。这个方法能复用图像模型，但很快会遇到两个问题：

| 单帧方案的问题 | 后续方法为什么出现 |
|---|---|
| 不知道不同帧里是不是同一个目标 | 检测 + 跟踪用于维持目标 ID 和轨迹 |
| 事件通常跨多帧发生 | 滑动窗口和时序模型用于判断过程 |
| 抽帧太稀会错过关键瞬间 | 需要抽帧策略和时间窗口设计 |
| 单帧标签对，整体事件仍可能错 | 需要评估时序稳定性和事件级指标 |

所以视频分析不是“图片模型跑很多次”就结束，而是从单帧识别升级到跨帧关联和事件理解。
