...
早期视觉系统如果只把人脸当作“一个类别”来检测，只能回答“这里有没有脸”。但身份相关任务还要回答“是不是同一个人”，这就需要更细的流水线：

| 阶段 | 解决什么历史问题 |
|---|---|
| 检测 | 先找出脸在图像里的位置 |
| 对齐 | 姿态、角度、裁切不同会让后续比对不稳定 |
| 特征提取 | 不直接记名字，而是把脸变成可比较的 embedding |
| 相似度匹配 | 通过阈值决定 same / different，处理误识和拒识 |

这也是人脸任务风险更高的原因：阈值、场景、光照和群体偏差都会影响身份判断，不能只展示“识别成功”的样例。
