...
这表示两个框的重叠不到它们合并区域的一半。在真实检测器里，这算不算匹配成功，要看你设定的 IoU 阈值，例如 `0.5` 或 `0.75`。
...
第一次做目标检测，不要只看一个 mAP 分数。先把错误拆开看：

| 错误类型 | 现象 | 优先检查 |
|---|---|---|
| 误检 | 没有目标的位置也画框 | 置信度阈值、负样本、背景相似物 |
| 漏检 | 真实目标没被框出来 | 小目标、遮挡、训练样本覆盖 |
| 框偏移 | 类别对了，但框不准 | 标注质量、IoU 阈值、输入缩放 |
| 类别混淆 | 框住了目标，但类别错 | 类别定义、相似类别样本、标签噪声 |
| 重复框 | 同一个目标出现多个框 | NMS 阈值、候选框过滤 |
| 小目标失败 | 大目标能识别，小目标不行 | 分辨率、特征层、数据增强策略 |

这个表比单个分数更有用，因为不同错误对应的修复方向完全不同。
---
