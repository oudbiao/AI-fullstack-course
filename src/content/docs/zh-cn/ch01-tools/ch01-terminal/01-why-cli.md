...
命令行最重要的能力不是记住十几个命令，而是把一次操作变成可复现记录。你可以把“我做了什么”写成命令，把“电脑返回了什么”保存成输出，这样别人才能帮你排查，也才能在服务器上重复同样的步骤。

后面训练模型、启动服务、提交 Git、连接 GPU 机器时，界面可能完全不同，但命令和输出仍然是共同语言。遇到错误时，先保留原命令、当前路径和完整报错，再决定下一步。
