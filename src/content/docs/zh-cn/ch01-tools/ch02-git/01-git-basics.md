...
这一页通过的标准不是“命令都敲过”，而是你能解释 Git 的三段状态如何流动。

最小证据应该包括：

1. 第一次提交后，`git log --oneline` 里有初始化提交。
2. 第二次修改后，`git diff` 能显示具体变化。
3. `git add` 后，`git status` 能说明文件已经进入暂存区。
4. 第二次提交后，历史里能看到两条 commit。
5. 每次提交后，`git status` 都应该回到 clean。

如果还有文件停在 modified 或 untracked，先不要急着学分支；回到 `工作目录 -> 暂存区 -> 提交历史` 这条主线重新检查。
</details>
