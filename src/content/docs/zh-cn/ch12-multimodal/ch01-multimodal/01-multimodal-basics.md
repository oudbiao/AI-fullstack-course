...
下面这些任务如果只看一种输入，很容易答偏：

| 真实任务 | 只看单一模态的问题 | 多模态补上的信息 |
|---|---|---|
| 客服截图问答 | 只看文字描述，不知道按钮、弹窗和错误位置 | 截图结构 + OCR 文本 + 用户问题 |
| 医学图文辅助 | 只看报告或只看影像都可能漏信息 | 影像区域 + 病历文字 + 检查结论 |
| 工业巡检 | 单张图片可能看不出设备状态变化 | 视频画面 + 传感器读数 + 时间序列 |
| 电商审核 | 只看标题可能不知道图片是否违规 | 商品图 + 标题 + 描述 + 类目 |

所以多模态不是“多接几个输入很酷”，而是当答案依赖多个证据来源时，系统必须把这些证据对齐到同一条判断链路里。
...
![多模态特征融合运行结果图](/img/course/ch12-multimodal-feature-fusion-result-map.webp)
...
3. 截图包含布局、颜色、报错位置、按钮状态和视觉上下文，这些信息在纯文本里会丢失。多模态系统可以把视觉线索和用户问题合在一起，回答哪里出了问题以及下一步该看哪里。
