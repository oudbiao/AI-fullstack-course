...
Stable Diffusion 的组件名字一样，但训练和推理的目标不一样：

| 阶段 | 输入是什么 | U-Net 学/做什么 | 输出是什么 |
|---|---|---|---|
| 训练时 | 真实图像压成的 latent、加噪后的 latent、文本条件、噪声步 | 学会预测噪声或去噪方向 | 让预测噪声接近真实噪声，更新参数 |
| 推理时 | 随机噪声 latent、文本条件、采样步数 | 按训练学到的方式一步步去噪 | 得到干净 latent，再由 VAE decoder 变成图像 |

所以推理时不是“拿一张真实图来训练”，而是从随机噪声开始生成。训练时才会用真实图像来教 U-Net：在某个噪声程度下，应该怎样往干净图像方向走。
