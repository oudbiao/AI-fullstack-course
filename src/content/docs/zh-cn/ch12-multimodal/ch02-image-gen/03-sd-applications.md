...
把 SD 用进真实项目时，生成前先过一遍这张清单：

| 检查项 | 要确认什么 |
|---|---|
| 版权 | 参考图、训练素材、风格要求是否有使用权限 |
| 肖像 | 是否涉及真实人物、公众人物或用户上传照片 |
| 敏感内容 | 是否可能生成暴力、色情、仇恨、误导性内容 |
| 模型来源 | 模型、LoRA、ControlNet 等权重来源是否可信 |
| 生成标识 | 是否需要标注 AI 生成、保存 prompt/seed/model 版本 |
| 人工复核 | 是否进入对外发布、商业使用或高风险场景 |

这张表不是拖慢创作，而是把“能生成”变成“能安全交付”。尤其是商业素材、人物肖像和品牌风格，不应只靠 prompt 自觉控制。
