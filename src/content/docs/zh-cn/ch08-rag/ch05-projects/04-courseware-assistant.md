...
```text
项目范围：事故主题 -> 政策检索 -> SOP schema -> Word 导出
中间产物：解析 chunks、检索命中、结构化 schema、模板 payload
来源追踪：政策、案例、清单项和外部说明都保留 source_refs
评估关口：检索、结构、引用、模板和失败处理检查
期望产出：SOP 草稿，以及每一层都可检查的证据
```
