...
文档解析失败通常不是程序直接报错，而是“看起来有文本，但结构已经坏了”。可以先按下面几类保存失败样本：

| 失败样本 | 现象 | 后续影响 |
|---|---|---|
| 两栏 PDF 顺序错 | 左栏第一段和右栏第一段混在一起 | chunk 语义断裂，检索命中也难回答 |
| 页眉页脚混入正文 | 每个 chunk 都带相同公司名或页码 | embedding 被噪声污染 |
| 表格被拉平成乱序文本 | 金额、条件、说明对应不上 | 生成答案可能引用错条件 |
| OCR 错字 | “退款”识别成相近但错误的字 | 关键词和向量检索都可能漏召回 |
| 标题层级丢失 | 小节不知道属于哪个主题 | metadata 过滤和引用变弱 |

保存这些失败样本，比只说“解析效果不好”更有用。后面你才能判断是换解析器、加规则清洗，还是先降低支持范围。
