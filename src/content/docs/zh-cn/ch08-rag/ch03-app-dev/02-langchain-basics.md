...
```text
请根据以下资料回答问题：资料=['课程购买后 7 天内可退款。']，问题=退款政策是什么
```
...
```text
模型输出：请根据以下资料回答问题：资料=['课程购买后 7 天内可退款。']，问题=退款政策是什么？
```
...
```text
{'answer': "请根据以下资料回答问题：资料=['课程购买后 7 天内可退款。']，问题=退款政策是什么？", 'ok': True}
```
...
LangChain 很适合原型和线性链路，但它不是每个项目的第一步。下面这些情况，先用普通函数把流程写清楚，往往更稳：
...
1. 给这个 `SimpleChain` 再加一步，把 查询 改写得更适合检索。
