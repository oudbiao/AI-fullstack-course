...
## RAG 为什么会出现：从检索到带证据生成

RAG 不是凭空出现的新概念，它更像是信息检索系统和大模型结合后的下一步。历史线索可以这样看：

```text
关键词搜索 -> 全文检索 -> 向量检索 -> 混合检索 -> RAG
```

| 阶段 | 解决了什么 | 还缺什么 |
|---|---|---|
| 关键词搜索 | 用精确词匹配快速找资料 | 同义词、改写问题、语义相近内容容易漏掉 |
| 全文检索 | 用倒排索引、BM25 等方法提升文档排序 | 仍然偏词面匹配，不会真正组织答案 |
| 向量检索 | 把问题和文档变成向量，用语义相似度找相关内容 | 找到了材料，但还需要人自己读和整合 |
| 混合检索 | 同时利用关键词和向量，提升召回稳定性 | 只能返回证据，不能直接完成可信回答 |
| RAG | 检索证据后交给 LLM 生成带引用的回答 | 需要切块、引用、评估和失败排查，不能只接一个向量库 |

这也是为什么本章不只讲“怎么调用模型”。RAG 的核心能力是让答案能沿着 document -> chunk -> retrieval -> answer -> citation 被复盘。

## 你在主线中的位置

你已经学过怎样用 Prompt、结构化输出和评估习惯控制大模型回答。这一章会加入外部知识：文档必须被解析、切块、检索、引用和测试，答案才值得信任。

这是从“模型能回答”走向“应用能基于正确证据回答”的桥。第 9 章会继续使用这种证据习惯，但系统还会选择工具、执行动作、读取观察结果，并留下执行 trace。

## 先看 RAG 应用闭环

![RAG 应用闭环](/img/course/ch08-rag-app-loop.webp)
...
RAG 答案不好时，不要先急着换模型。先沿着“资料怎样进入答案”的路径逐层检查：

```text
文档解析 -> 切块 -> 向量化 / 入库 -> 检索召回 -> 重排 / 过滤 -> 上下文拼接 -> 生成回答 -> 引用与评估
```

| 层 | 先确认什么 | 必须打印或保存什么 | 常见修复 |
|---|---|---|---|
| 文档解析 | 原文有没有被正确读出来 | 文件名、标题、页码、抽取文本片段 | 换解析器，处理 OCR、表格、编码和空页 |
| 切块 | chunk 是否保留完整事实 | `chunk_id`、文本、source、section、page | 调整 chunk 大小、overlap、标题继承和 metadata |
| 向量化 / 入库 | 记录是否完整入库 | 记录数、向量维度、metadata 字段 | 统一 embedding 模型，补 metadata，去重或重建索引 |
| 检索召回 | 正确证据有没有进入 top-k | query、top-k、score、source、text | 改写 query，混合检索，调整 top-k 或 embedding |
| 重排 / 过滤 | 好证据有没有排到前面 | rerank 前后排序、过滤条件 | 加 reranker，修正 metadata filter，放宽过严条件 |
| 上下文拼接 | 证据有没有进入最终 Prompt | 最终 Prompt、context 片段顺序、token 长度 | 控制上下文长度，保留来源，删除低相关片段 |
| 生成回答 | 模型有没有使用证据 | 答案、引用、无法回答原因 | 强化“只基于资料回答”，加入无依据兜底 |
| 引用与评估 | 结果能不能复盘 | 固定评估集、期望来源、失败标签 | 增加回归测试，记录错例，比较 before / after |

一个实用习惯是：每次改 RAG，只改一个变量，并用同一组问题复测。否则你很难知道变好是因为切块、检索、Prompt，还是只是碰巧。

## 学习顺序与任务表

完整工作坊放在基础之后。先看见检索链路，再把它封装成应用。优先走核心路径：**8.1 -> 8.3 -> 8.4 -> 8.5**。当需要本地服务、统一 API 或部署选择时，再学习 8.2。
...
| 8.4 | 工程实践 | 加运维说明和上下文策略 | 日志、配置、部署清单、策略卡 |
| 8.5 | 阶段项目 | 运行 [8.5.6 实操：第 8 章 RAG 应用完整工作坊](/zh-cn/ch08-rag/ch05-projects/05-stage-hands-on-workshop/) | 工作坊输出、一个新增文档、一个新增评估问题 |
| 8.2 | 部署与统一 API | 理解云 API、本地模型、统一调用层 | 一份调用笔记或配置对比 |

## 必修主线、扩展和深度挑战

- **必修核心**：文档解析、chunk metadata、top-k 检索、引用、无法回答处理、固定评估集和请求/响应日志。这是可信知识增强大模型应用的最小能力。
...
可打印清单见 [8.0 学习检查表](/zh-cn/ch08-rag/study-guide/)。如果想直接做项目，从 [8.5.6 实操：第 8 章 RAG 应用完整工作坊](/zh-cn/ch08-rag/ch05-projects/05-stage-hands-on-workshop/) 开始。
