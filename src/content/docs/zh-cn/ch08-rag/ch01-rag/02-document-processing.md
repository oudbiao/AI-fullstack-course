...
![SOP 文档知识块元数据 schema 图](/img/course/ch08-courseware-chunk-metadata-schema-map.webp)

:::tip[读图提示]
SOP 文档生成最怕“找到了文字却不知道该放进哪个栏目、承担哪种证据角色”。看图时重点关注 `topic`、`content_type`、`source_origin`、`page_or_slide` 这几个字段，它们会决定后面能不能按政策、案例、清单和引用稳定组装。
:::
...
sop_chunks = [
    ...
    },
    {
        ...
        "section": "处理案例",
        "page": 2,
        "text": "一次失败结账后客户被扣款两次，客服核对两笔扣款后升级给 billing。",
    },
    {
        "topic": "退款升级",
...
| 按内容类型切 | 政策 / 案例 / 清单分开 | 适合生成 SOP 文档 | 需要解析或标注 | 适合结构化项目 |
...
4. 如果你的目标是生成 SOP 文档，想一想：政策、处理案例、复核清单为什么不适合完全用同一种切块方式？
...
3. 优先按语义边界切分，再用 overlap 或 parent-child chunks 处理跨段条款。目标是让每个可检索单元都包含足以支撑回答的信息。
4. 政策需要完整条件和例外，案例需要证据和结果，清单需要清楚的操作检查。统一切法可能会切断决策证据，也可能让政策检索变得太噪。
