...
- **没有命中**：知识库里有答案，但 top-k 没找出来。先检查 chunk 大小、关键词不匹配和 embedding 是否适合，再打印 top-k，对照 query 与 chunk 文本。
- **漏掉条件**：命中了正确文档，但答案漏掉关键限制。先检查 chunk 是否完整、上下文顺序和 prompt 约束，再增加 overlap、调整上下文组装或要求引用条件。
- **引用不支撑结论**：答案引用了来源，但来源没有支持结论。先检查生成阶段幻觉和引用拼接，再逐句核对证据。
- **来源冲突**：多个文档互相冲突，答案变混乱。先检查版本、日期和来源优先级，再加 metadata filter 和来源优先级规则。
