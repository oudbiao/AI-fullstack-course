...
head:
  - tag: meta
    attrs:
      name: keywords
      content: "贝叶斯法则, 最大似然估计, EM算法, 香农信息论, AI数学历史"
---
...
- 复盘通过的关键，是能把每个数字翻译成判断：什么改变了，什么仍然不确定，什么新证据会更新结论。
- 至少检查一次 Bayes 更新、一次似然选择、一次熵或交叉熵结果。如果只有公式，没有通俗解释，就还没完成。
- 保留一个把概率当成确定性，或忽略基率的失败案例。这是概率在实际项目中最常见的坑。
- 当你能解释为什么概率是数据分析、机器学习训练、损失函数和模型评估背后的共同语言时，本页就算通过。
