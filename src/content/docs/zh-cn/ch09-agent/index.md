...
| 9.10 | 阶段项目 | 运行 [9.10.5 实操：构建一个可追踪的单 Agent 助手](/zh-cn/ch09-agent/ch10-projects/04-stage-hands-on-workshop/) | `agent_traces.jsonl`、安全边界、评估用例 |
...
| 节奏 | 完成什么 | 作品集交付物 |
|---|---|---|
| 快速通过 | 一个可追踪 Agent 闭环，并拦截一个不安全工具 | `agent_traces.jsonl` 和一段追踪说明 |
| 标准完成 | 核心路径 9.1 -> 9.2 -> 9.3 -> 9.4 -> 9.8 -> 9.10 | `tools_schema.md`、`safety_boundary.md`、评估用例和失败 trace |
| 深度完成 | 单 Agent 稳定后，再增加 MCP、框架比较、部署准备或多 Agent 协作 | 设计备忘，说明为什么 Agent 比 workflow/RAG/function calling 更安全或更有用 |
...
可打印清单见 [9.0 学习检查表](/zh-cn/ch09-agent/study-guide/)。如果想直接做项目，从 [9.10.5 实操：构建一个可追踪的单 Agent 助手](/zh-cn/ch09-agent/ch10-projects/04-stage-hands-on-workshop/) 开始。
