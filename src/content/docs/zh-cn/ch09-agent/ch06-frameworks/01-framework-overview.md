...
| 现象 | 更适合先手写 | 更适合引入框架 |
|---|---|---|
| 状态流 | 只有 1-2 个固定步骤 | 有分支、循环、重试和回放需求 |
| 工具数量 | 只有少量稳定工具 | 工具多，且需要统一注册、权限和错误处理 |
| 调试方式 | 打印中间变量就够 | 需要 trace、运行记录和可视化状态 |
| 团队协作 | 一个人能看完整流程 | 多人维护不同节点、工具或角色 |
| 变化速度 | 需求还没定型 | 流程基本稳定，开始重复开发相同结构 |

如果你还说不清 Agent 的状态、工具、输入输出和失败路径，先不要急着选框架。把这些边界画出来后，框架选择会自然很多。
