...
- 检索漏召回
  关键资料没有进入候选。检查查询是否太窄、关键词是否不匹配、`top_k` 是否太小。可以用查询改写、混合检索、扩大候选后 rerank 来改进。

- 阅读不完整
  命中文档是对的，但漏掉关键段落。检查 chunk 大小和上下文组装。可以用 parent-child retrieval 或重新调整上下文拼装。

- 总结过度概括
  摘要听起来对，但丢了限制条件。改进 prompt，要求每条输出都保留 claim、condition 和 source。

- 引用不支持
  claim 和 source 对不上。增加 citation check，并逐条验证 claim。

- 循环调用
  Agent 一直检索不停止。增加最大步数，并在没有新增信息时停止。
