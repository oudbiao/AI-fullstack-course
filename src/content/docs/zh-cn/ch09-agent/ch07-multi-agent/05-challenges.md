...
多 Agent 不一定比单 Agent 强。最常见的反例是：为了显得“分工明确”，系统加了很多角色，但没有定义交接物。

```text
planner: 说要检索退款政策
researcher: 检索了公开政策，但没保存 source
writer: 自己又凭印象写了一版
reviewer: 只检查语气，没有检查证据
manager: 汇总时不知道谁的结果可信
```

这个失败不是因为某个 Agent 不努力，而是因为缺少三件事：

| 缺失 | 后果 |
|---|---|
| 明确交接物 | 每个 Agent 都在自由发挥，结果无法合并 |
| 单一负责人 | 冲突时没人拍板，系统不收敛 |
| 共享 trace | 出错后不知道是哪一步开始偏离 |

所以引入多 Agent 前，先问一句：单 Agent + 工具 + 审核清单是否已经不够？如果还没有证明“不够”，先不要为了角色感而拆角色。
