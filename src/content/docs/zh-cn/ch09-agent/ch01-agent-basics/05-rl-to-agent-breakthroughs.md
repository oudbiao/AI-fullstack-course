...
- 复盘通过的关键，是能把一个历史系统对应到一个 Agent 设计选择：自我对弈对应反馈数据，DQN 对应环境行动，AlphaGo 对应搜索和规划，RLHF 对应偏好塑形。
- 画出或写出一个包含目标、状态、行动、观察、评估器和安全约束的循环。任何一块缺失，Agent 都很难可靠复盘。
- 保留一条 Agent 做错动作的 trace，并说明问题来自目标、状态、工具权限、规划器还是评估器。
- 当你能解释“可追踪 Agent 是受控行动系统，而不只是多写了指令的聊天机器人”时，本页就算通过。
