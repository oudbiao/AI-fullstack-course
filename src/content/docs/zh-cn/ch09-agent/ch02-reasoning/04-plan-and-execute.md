---
title: "9.2.5 计划-执行模式（Plan-and-Execute）"
...
> **先用 规划者 拆计划，再由 executor 按计划逐步完成。**
...
### 什么时候不适合 Plan-and-Execute？

| 场景 | 为什么不适合 | 更稳的方式 |
|---|---|---|
| 任务只有一步两步 | 规划层成本大于收益 | 直接调用工具或普通 workflow |
| 每一步都强依赖实时观察 | 计划很快过期 | ReAct 或带监控的动态重规划 |
| 目标本身还没定义清楚 | planner 会产出漂亮但错误的计划 | 先澄清需求和验收标准 |
| 高风险动作不能自动执行 | executor 可能按错计划执行副作用 | 人类审批后再执行 |

Plan-and-Execute 的价值在于减少长任务漂移，不是把所有任务都变成“先写计划”。计划越长，越要明确何时重规划、何时停下、何时交给人审。
