...
```text
项目名称：课程知识助手，或你自己的替代项目
章节成长规则：每章只加一个能力，不堆一堆 demo
重跑路径：README 命令、脚本、notebook cell，或服务端点
审阅包：数据说明、评估用例、trace、失败笔记和 release note
期望产出：一条从环境搭建成长到 RAG、Agent 和运行时证据的项目线
```
