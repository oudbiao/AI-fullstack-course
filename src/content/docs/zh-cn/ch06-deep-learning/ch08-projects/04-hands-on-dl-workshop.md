...
读这张图时，重点看训练证据从 batch shape 开始，到训练日志、曲线、checkpoint、复盘样本和 README 结束。没有这些输出，只有一行 accuracy 很难判断训练是否健康。
...
- `outputs/training_log.csv`：每个 epoch 的训练和验证指标。
- `outputs/model_comparison.csv`：baseline 和 CNN 的对比。
- `outputs/confusion_matrix.csv`：类别级错误分布。
- `outputs/error_samples.csv`：错误样本或低置信度复盘样本。
- `outputs/metrics_summary.json`：最终模型和测试指标摘要。
- `curves/loss_curve.png`：训练/验证 loss 曲线。
- `checkpoints/best_model.pt`：可加载的最佳模型状态。
- `reports/shape_trace.md`：batch、label 和 logits 的 shape 记录。
- `reports/debug_checklist.md`：排错清单。
...
先读执行顺序图：目录、脚本、第一次运行、证据检查要按顺序做。第一轮先不要调参，它是后面实验的参照线。

![Dataset 到 DataLoader 批处理流程](/img/course/ch06-hands-on-dataset-dataloader-batch-flow.webp)

再读批处理图：`Dataset` 负责单条样本，`DataLoader` 负责打包成 batch，训练循环只接收已经整理好的 batch。
...
读这张输出图时，不要从最终 test accuracy 开始。先看 shape trace，再看训练曲线和模型对比，最后才读错误样本和 checkpoint。
...
这张作品集图提醒你：一个能被别人审阅的深度学习项目，至少要包含训练日志、曲线、checkpoint、错误样本和复跑说明。

![PyTorch 小步重跑实验闭环](/img/course/ch06-hands-on-rerun-experiment-loop.webp)

读复跑闭环图时，只记住一个纪律：一次只改 `batch_size`、学习率或 epoch 里的一个变量，再用同一套输出判断变化。
