...
第一次学 LSTM，不要先背 gate 名字。先把它想成一本会自动更新的长期笔记：

```text
旧笔记 -> 擦掉不重要的内容 -> 写入当前重要信息 -> 读出当前要用的摘要
```
...
| 写入新内容 | input gate |
| 读出摘要 | output gate |
| 当前对外摘要 | `hidden state h_t` |

有了这个直觉后，再看公式会容易很多：gate 不是神秘开关，而是在每个时间步决定记忆怎么擦、怎么写、怎么读。
...
## LSTM/GRU 为什么从普通 RNN 后面出现

普通 RNN 每个时间步都用新输入重写 hidden state。序列越长，越早的信息越像被一遍遍转述，容易变淡；反向传播也要穿过很多重复步骤，梯度容易变弱或不稳定。

门控模型补上的能力是“可学习的记忆管理”：

| 普通 RNN 的历史问题 | LSTM / GRU 的回答 |
|---|---|
| 早期信息反复被覆盖 | gate 学习哪些旧信息继续保留 |
| 所有新信息都混进 hidden state | gate 学习当前信息写入多少 |
| 长距离训练信号回传困难 | 更稳定的记忆通道缓解梯度衰减 |
| 只靠一个 hidden state 承担全部记忆 | LSTM 显式拆出 `cell state`，GRU 用更轻结构混合旧新状态 |

所以先记住历史位置：普通 RNN 让模型按顺序读数据，LSTM/GRU 则让模型更有选择地记住长序列里的关键信息。
...
```text
LSTM 状态：返回隐藏状态 h 和细胞状态 c
GRU 状态：只返回隐藏状态 h
门控含义：接近 0 的值会阻断，接近 1 的值会放行
记忆任务：标签取决于第一个时间步
结果：final_acc 在 toy memory 任务上达到 1.0
限制: 在信任该架构前先用留出序列验证
```
