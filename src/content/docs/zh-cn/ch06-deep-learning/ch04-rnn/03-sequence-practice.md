...
:::note[这个实验的边界]
这个实验证明：滑动窗口、时间顺序切分、LSTM 预测和 naive baseline 对比可以组成一个诚实的序列预测闭环。

它还不能证明：LSTM 是所有时间序列的最佳选择。真实项目还要处理缺失值、节假日、外部变量、多步预测和 rolling validation。
:::
...
3. 移除梯度裁剪可能导致 loss 偶发尖峰或不稳定，尤其是学习率偏大、序列更长时。
4. 增加第二个特征后，输入 shape 的最后一维会从 1 变成 2，模型的 `input_size` 也要同步修改。
5. Rolling forecast 会把误差带入后续窗口，所以越往后预测越容易漂移。它检验的是真实部署中的多步预测能力。
