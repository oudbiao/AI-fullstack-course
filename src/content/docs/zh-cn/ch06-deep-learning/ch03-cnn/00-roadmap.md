...
在 CNN 之前，也可以把一张图片摊平成一个很长的向量，再接全连接网络。但这样会遇到三个问题：

| 旧做法的问题 | CNN 的回答 |
|---|---|
| 图片像素很多，全连接参数量太大 | 小 kernel 只看局部区域，参数更少 |
| 图像有空间结构，摊平后上下左右关系变弱 | 卷积保留高、宽和通道结构 |
| 同一种边缘或纹理可能出现在不同位置 | 权重共享让同一个滤波器在整张图上扫描 |

所以 CNN 的历史意义不是“专门给图片换一种层”，而是把图像里的局部性、平移模式和层级特征变成可训练结构。早期层常学边缘和纹理，后面层再组合成更复杂的形状。
