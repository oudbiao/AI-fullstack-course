...
Prompt 的历史位置很简单：当预训练模型已经有能力时，最快的适配方式不是训练，而是把任务、上下文、格式和边界写清楚。

| 你遇到的问题 | 先试 Prompt 的原因 | 什么时候继续升级 |
|---|---|---|
| 任务没说清，回答发散 | Prompt 可以补任务简报 | 固定案例仍长期失败 |
| 输出格式不稳定 | Prompt + schema 可以约束结构 | 解析失败率仍高 |
| 需要少量风格示例 | Few-shot 示例成本低 | 风格要求长期稳定且样本多 |
| 需要最新事实 | Prompt 只能放有限上下文 | 转向 RAG，不是直接微调 |

所以 Prompt 是最轻的应用接口，也是后面 RAG、微调和对齐的基线。没有可复现 Prompt 基线，就很难判断训练是否真的带来了收益。
