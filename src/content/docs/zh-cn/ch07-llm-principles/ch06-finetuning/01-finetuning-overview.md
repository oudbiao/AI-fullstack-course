...
每次想“要不要训练”时，先把方案放到这条阶梯上：

| 阶梯 | 是否改参数 | 适合解决 | 不适合解决 |
|---|---|---|---|
| Prompt | 不改 | 任务说明、格式约束、少量示例 | 大量稳定行为漂移 |
| RAG | 不改 | 最新知识、私有知识、可引用依据 | 固定语气和任务习惯 |
| 工具/工作流 | 不改 | 查询、计算、动作执行 | 模型本身输出风格 |
| SFT / 微调 | 改 | 稳定任务格式、领域表达、固定风格 | 快速变化的知识库 |
| RLHF / DPO | 改 | 偏好、帮助性、诚实性、安全边界 | 没有稳定偏好数据的任务 |
| LoRA / QLoRA | 改少量 | 低成本训练稳定行为 | 用训练替代检索或评估 |

这张表的目的不是阻止训练，而是防止把所有问题都推给微调。越往下，成本、评估压力和回滚风险越高。
