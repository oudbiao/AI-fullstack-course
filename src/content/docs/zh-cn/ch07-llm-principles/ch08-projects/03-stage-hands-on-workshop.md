...
head:
  - tag: meta
    attrs:
      name: keywords
      content: "LLM 实操工作坊, Prompt 评测, 结构化输出, 微调决策, 章节项目"
---
...
<details>
<summary>工作坊复盘要点与通过标准</summary>

- 工作坊通过的标志是：保存下来的证据文件夹不依赖终端历史，也能解释这次运行。
- 改 Prompt 前先看 `prompt_eval.csv`。下一步最好来自固定失败案例，而不是来自主观感觉。
- 把 `failure_cases.md` 当作后续 RAG 或微调决策的桥。如果失败来自缺知识，先试检索；如果失败来自格式漂移，先收紧结构化输出。
- 当你能重跑脚本、检查证据文件，并说明下一步为什么是 Prompt、RAG、微调或暂不修改时，本页就算通过。
