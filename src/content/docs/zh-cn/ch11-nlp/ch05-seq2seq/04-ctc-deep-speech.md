...
CTC 解决的是“没有逐帧对齐”的训练问题，但它不保证输出一定完美。常见失败可以先这样分：

| 失败类型 | 例子 | 可能原因 |
|---|---|---|
| 重复没折干净 | `我 我 爱 AI` | 解码后处理或语言约束不足 |
| 字被吞掉 | `我 AI`，漏掉“爱” | 音频帧弱、噪声大、目标太短 |
| blank 太多 | 输出很短甚至空 | 模型过度偏向 blank |
| 同音/近音混淆 | `爱` 变成 `哎` | 声学特征不足或语言模型弱 |

所以 ASR 项目不能只看“能不能出字”。还要保留音频片段、预测文本、真实文本和错误类型，才能判断是声学问题、解码问题，还是语言约束问题。
