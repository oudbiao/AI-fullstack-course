...
如果你已经学过第 7 章，不需要把这里当成重复内容。第 7 章是从 LLM 应用视角看 token、embedding 和模型调用；第 11 章是从 NLP 任务视角看文本、标签、实体、表示、模型家族和评估证据。遇到相同术语时，重点换成“这个文本任务应该怎样定义和验证”。

如果你走最快的新手路线，先完成第 1-9 章，再回来做一个文本方向作品集项目。

## NLP 为什么一路演进到大模型

NLP 的历史可以先读成一条“让机器越来越少依赖人工规则”的路线：
...
| 阶段 | 解决什么 | 卡在哪里 | 后面为什么继续演进 |
|---|---|---|---|
| 规则/词典 | 快速处理明确模式，例如关键词、正则、词典匹配 | 覆盖不了大量表达变体 | 需要从数据里统计规律 |
| n-gram / TF-IDF | 用词频和共现建模文本 | 稀疏、看不到深层语义和长距离关系 | 需要更紧凑的语义表示 |
| word2vec 类词向量 | 让相似词在向量空间更接近 | 一个词通常只有一个固定向量 | 需要根据上下文改变词义 |
| RNN / CRF / Seq2Seq | 处理序列、实体边界和文本到文本任务 | 长距离依赖和并行训练困难 | 需要更强的上下文建模 |
| BERT / GPT / T5 | 先学通用语言能力，再迁移到任务 | 仍需任务定义、评估和工程边界 | LLM 把多种任务统一到提示和生成接口 |

所以本章不是把旧 NLP 重新学一遍，而是补上 LLM 下面的任务地基：文本怎样被切成单位、怎样表示语义、怎样定义标签和实体、怎样评估生成或抽取是否可靠。
