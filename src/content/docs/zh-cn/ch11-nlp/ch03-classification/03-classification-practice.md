...
## 五、error analysis 应该怎么做？

文本分类不要只看总准确率。至少把错误拆成下面几类：

| 检查项 | 看什么 | 可能动作 |
|---|---|---|
| 错分样本 | 哪些文本 `gold != pred` | 保存原文、gold、pred、分数和原因 |
| 标签重叠 | 两个标签是否本来就边界模糊 | 重写标签定义，增加混淆反例 |
| 类别不平衡 | 某些类别样本是否太少 | 补数据、重采样或看 macro F1 |
| 阈值问题 | 低置信度样本是否被硬分类 | 增加“不确定/转人工”策略 |
| 宏/微 F1 差异 | 大类好、小类差是否被平均掩盖 | 同时报告 macro F1 和 per-class 指标 |

一个实用结论是：如果 accuracy 看起来不错，但小类别几乎全错，项目仍然可能不可用。文本分类项目最应该保存的是“错在哪里”，不是只保存“分数是多少”。
