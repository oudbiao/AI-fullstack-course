...
:::note[過去の URL と素材名]
このページは以前 courseware assistant プロジェクトでした。現在の表示内容は SOP 文書アシスタントに再構成されていますが、古いリンクを壊さないため、slug と一部の画像ファイル名には意図的に `courseware` を残しています。将来リンク移行を行う場合は、slug、画像 stem、リダイレクトを同時に更新してください。
:::
