...
head:
  - tag: meta
    attrs:
      name: keywords
      content: "RAG 実践, RAG ワークショップ, LLM アプリプロジェクト, 引用, RAG 評価, Responses API"
---
...
## ステップ 2：完全なオフライン RAG スクリプトをコピーする
...
## ステップ 6：フレームワークを足す前に トレース 思考を作る

![アシスタント会話とツール トレース 図](/img/course/ch08-assistant-session-tool-trace-map-ja.webp)

実際の LLM アプリでは、trace は 1 回のリクエストで何が起きたかの記録です。まだログファイルを保存しなくても、次の流れは説明できるようにします。

| トレース 段階 | このスクリプトでの場所 | 後で記録したいもの |
|---|---|---|
| 入力 | `query`、`role` | ユーザー ID、セッション ID、リクエスト ID |
| 解析 | `chunk_documents()` | 文書バージョン、パーサー名 |
| 検索 | `retrieve()` | top-k chunk、スコア、クエリ書き換え |
| 権限 | `allowed_hits`、`blocked_hits` | ロール、ポリシー、ブロックされた source 数 |
| 回答 | `build_answer()` | 状態、引用、モデル名 |
| 評価 | `evaluate()` | 合格/失敗、失敗理由 |

だから第 8 章は、Prompt だけではなくアプリケーションエンジニアリングなのです。信頼できるシステムには、途中状態が見える必要があります。

## ステップ 7：embedding、ベクトルデータベース、API への拡張ルート
...
## ステップ 8：任意の OpenAI Responses API 拡張
...
## ステップ 9：関数呼び出し と構造化出力の考え方

![関数呼び出し の検証とディスパッチ図](/img/course/ch08-function-calling-validation-dispatch-map-ja.webp)
...
| 段階 | 起きること | 安全ポイント |
|---|---|---|
| スキーマ | ツール入力フィールドを定義する | 足りないフィールドや未知のフィールドを拒否する |
| 検証 | ロール、source、許可された操作を確認する | モデルの引数をそのまま信じない |
| 実行分岐 | 実際の関数を実行する | 副作用を制御する |
| 観察 | 結果をモデルに返す | 先に private データをフィルタする |
| 最終回答 | 引用付き回答または回答不能状態を返す | 表示前に検証する |

オフラインスクリプトも同じ習慣を練習しています。検索、権限、回答、評価は分けて扱います。

## ステップ 10：よくあるエラーのチェックリスト
...
| レベル | タスク | 合格基準 |
|---|---|---|
| 入門 | public 文書を 1 つ、評価ケースを 1 つ追加する | `passed` 数が増え、新しい引用が表示される |
| 標準 | `logs/retrieval_logs.jsonl` を出力する | 各質問に クエリ、role、status、score、citations が記録される |
| 標準 | `top_k` 設定変数を追加する | `top_k=1` と `top_k=2` の結果を比較できる |
