...
| 検索層 | 質問に最も関係するチャンクを探す | クエリ、top-k チャンク、スコア、source ID |
...
| 8.4 | 工程実践 | 運用メモと context strategy を足す | ログ、設定、配置チェックリスト、strategy card |
| 8.5 | ステージプロジェクト | [8.5.6 実践：第 8 章 RAG アプリ完全ワークショップ](/ja/ch08-rag/ch05-projects/05-stage-hands-on-workshop/) を動かす | ワークショップ出力、追加文書1つ、追加評価ケース1つ |
| 8.2 | 配置と統一 API | クラウド API、ローカルモデル、統一呼び出し層を理解する | 呼び出しメモまたは設定比較 |

## 必修ルート、拡張、深掘り

- **必修コア**: 文書解析、chunk metadata、top-k 検索、引用、回答不能処理、固定評価セット、request/response ログです。信頼できる知識接地 LLM アプリの最小スキルになります。
- **任意の拡張**: ローカルモデル serving、統一 API、LangChain/LlamaIndex、高度な RAG、Docker 配置です。規模、フレームワーク統合、運用深度が必要なときに戻ります。
- **深掘り課題**: 同じ評価質問を固定し、検索または chunking 変数を1つだけ変え、引用付き回答を比較します。感覚だけの RAG 調整を防ぎます。

## 時間配分と提出物

| ペース | 完了すること | ポートフォリオ提出物 |
|---|---|---|
| Fast pass | Tiny RAG、top-k printout、引用付き回答1つ、回答不能ケース1つ | query、chunks、answer、failure note を含む `rag_trace.md` |
| Standard pass | 中核ルート 8.1 -> 8.3 -> 8.4 -> 8.5 | API request/response、eval case、追加 document を含む README section |
| Deep pass | local serving、reranking、framework integration、Docker のいずれか1つを追加 | before/after eval table、latency/cost note、deployment checklist |

強い第 8 章の成果は chatbot screenshot ではありません。document -> chunk -> retrieval -> answer -> citation -> evaluation という再実行可能な evidence path です。

## 最初に動かすループ：フレームワークなしのミニ RAG（Tiny RAG）
...
| 深い確認 | document、chunking、retrieval、reranking、generation、citation、レイテンシ、cost の失敗を分けられる。 |
...
- チェーンを理解する前にフレームワークへ進む。クエリ、chunks、prompt、answer、source を表示できてからの方が学びやすいです。
...
印刷用チェックリストは [8.0 学習チェックリスト](/ja/ch08-rag/study-guide/) を使ってください。プロジェクトから始めたい場合は [8.5.6 実践：第 8 章 RAG アプリ完全ワークショップ](/ja/ch08-rag/ch05-projects/05-stage-hands-on-workshop/) へ進みます。
