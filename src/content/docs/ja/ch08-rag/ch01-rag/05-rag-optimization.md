...
## 八、目標が「知識ベース駆動の SOP 文書アシスタント」なら、最適化の順番はどうする？
...
1. まず文書解析が正しいかを見る
2. 次に知識ブロックがポリシー / 対応ケース / チェックリストに分かれているかを見る
3. それから、検索で正しい内容を召回できているかを見る
4. さらに、構造化出力とテンプレートがポリシー、ケース、チェックリストを正しい位置に入れているかを見る
...
## 九、SOP 文書プロジェクトに近い最小の最適化チェックリスト

| 現象 | まず確認したほうがよい場所 |
|---|---|
| テーマは合っているのに対応ケースがない | 文書解析 / コンテンツタイプのラベル付け |
| ケースは見つかったのに、ポリシー欄に入っている | スキーマ / テンプレートのマッピング |
...
- **正しい資料が出ない**: まずクエリ、top-k の元ヒット、chunk テキストを見ます。切り分け、キーワード検索、クエリ書き換えを試し、いきなり大きい生成モデルに変えないようにします。
- **正しい資料の順位が低い**: まず各 chunk の score と順位を見ます。rerank や混合検索の重みを試し、理由なく top-k を大きくしすぎないようにします。
- **コンテキストにあるのに条件が抜ける**: まず最終コンテキスト、prompt、回答の引用を見ます。コンテキスト構成と引用指示を調整し、embedding モデルだけを変えないようにします。
- **間違ったソースを引用する**: まず answer、`source_refs`、証拠の断片を見ます。citation check と引用形式の制限を試し、最終回答の流暢さだけを見ないようにします。
- **遅延とコストが急に上がる**: まず top-k、rerank 数、コンテキスト長を見ます。候補数制限、キャッシュ、段階的検索を試し、top-k とモデルサイズを同時に増やさないようにします。
...
- `baseline`: キーワード検索、`top-k=3`。専門用語では安定しますが、言い換え質問に弱いです。比較対象として残します。
- `exp-1`: クエリ書き換えを追加。言い換え質問のヒット率は上がりますが、誤った書き換えも少し出ます。書き換えログを残せる場合だけ採用します。
- `exp-2`: rerank を追加。正しい資料が上位に来やすくなりますが、遅延は増えます。遅延が許容できるなら標準版にします。
