...
head:
  - tag: meta
    attrs:
      name: keywords
      content: "AI支援コーディング, コード生成, コードレビュー, テスト, デバッグ, LLM開発"
---
:::tip[この節の位置づけ]
...
![AI 支援コーディングのテスト実行結果図](/img/course/ch08-ai-coding-test-run-result-map-ja.webp)
...
1. 良いテストには、通常ケース、境界ケース、失敗/不正入力ケースが含まれます。テストが走るだけでなく、assertion が妥当かを確認します。
2. 最小修正は失敗している行または契約に絞り、広い refactor を避けます。
3. 合格基準付き Prompt は、命名、境界処理、テストしやすさを改善し、モデルの勝手な振る舞いを減らしやすいです。
4. README には、AI が生成したもの、自分が変更したもの、実行したテスト、既知の制約、review 判断を書きます。
