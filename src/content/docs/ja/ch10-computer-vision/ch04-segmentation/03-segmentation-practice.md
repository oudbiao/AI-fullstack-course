...
![分割プロジェクトの失敗サンプル分類図](/img/course/ch10-segmentation-practice-failure-buckets-map-ja.webp)
...
1. `pred_masks` を変えると、mask の欠落、クラス誤り、境界の悪さによって `mean_iou` は下がります。簡単なクラスが別クラスの失敗を隠さないよう、class-wise IoU も確認します。
2. mask annotation standard が重要なのは、境界の太さ、ignore 領域、あいまいな edge が、そのまま学習目標と評価基準になるからです。
3. とても小さい領域では、数画素の誤りでも intersection と union の比率が大きく変わるため、IoU は特に敏感です。
4. ポートフォリオページでは、問題定義、mask ルール、IoU/Dice 指標、overlay 例、失敗例、制約、次の改善を示します。
