---
title: "2.3.1 プロジェクト：コマンドラインタスクマネージャー"
description: "Python の基礎知識を総合的に活用して、コマンドラインのタスク管理ツールを作る"
sidebar:
  order: 1
---
![コマンドラインタスクマネージャーのアーキテクチャ図](/img/course/ch02-todo-cli-architecture-ja.webp)
...
|---|---|
| メニュー | 表示、追加、完了、削除、終了 |
| ユーザー操作 | `1` を選んで全タスクを表示 |
| タスクリスト | 3 件のタスク、1 件は完了済み |
| 要約 | `合計 3 件のタスク、完了済み 1 件` |
...
class TaskManager:
    ...
    def load(self) -> None:
        ...
        self.tasks = []
        for item in data:
            task = Task(item["title"], item.get("priority", "中"))
