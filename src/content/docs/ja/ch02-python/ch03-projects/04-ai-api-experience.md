...
response = client.responses.create(
    model="gpt-4o-mini",
    instructions="あなたは Python エンジニアリングアシスタントです。明確に説明し、すぐ実行できる次の一手を1つ示してください。",
    input=[
...
さまざまなエンジニアリング役割の AI アシスタント（Python メンター、コードレビュー支援、API デバッグ支援、面接コーチ）を作り、ユーザーに選んでもらう。
...
1. ストリーミングでは `response.output_text.delta` イベントを順に処理し、各チャンクをすぐ表示して、返答が少しずつ見えるようにします。
2. まず直接の CLI 版が動いてから FastAPI でチャットボットを包むと、CLI と HTTP の挙動を比較しやすくなります。
3. ロールプレイでは、同じ会話ループを保ったまま、prompt や system instruction を切り替える役割セレクタを作ります。各役割は、より明確な説明、レビューコメント、デバッグチェックリスト、面接フィードバックなど、異なるエンジニアリング成果物を返すようにします。
4. ローカル知識ベースでは、先にノートや Markdown を読み込み、関連箇所を検索して、その内容を踏まえて回答します。出典スニペットを見せると答えの信頼性が上がります。
5. 自己チェック: API Key が安全に保存されていること、マルチターン記憶が動くこと、少なくとも1つの実用ツールが動くこと、キーがハードコードされていないことを確認します。
</details>
