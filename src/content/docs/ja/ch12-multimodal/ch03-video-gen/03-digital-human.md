...
![デジタルヒューマンのマルチモジュール同期図](/img/course/ch12-digital-human-sync-pipeline-map-ja.webp)
...
![デジタルヒューマン pipeline list の実行結果図](/img/course/ch12-digital-human-pipeline-list-result-map-ja.webp)
...
3. 教育用の仮想講師には、script または対話生成、TTS、avatar rendering、lip sync、授業状態、検索またはカリキュラム grounding、安全レビュー、学習進捗分析が必要です。
4. 安定性とコストが重視されるのは、デジタルヒューマンが長時間かつ繰り返し使われることが多いからです。少し写実性が低くても、信頼できて安価なシステムの方が、不安定で高価なシステムより導入しやすいです。
