---
title: "A.2 コース番号の約束"
description: "ソースディレクトリ ch01-tools、ch02-python などの番号と、Web 上で表示する第 1～13 章との対応関係を説明し、コースを保守する際の混乱を避けます。"
head:
  - tag: meta
    attrs:
      name: keywords
...
:::
コースのWebページは、学習者向けに第 1～13 章の表示番号で統一しています。ソースディレクトリも表示章番号に合わせてあり、`ch01-*` は第 1 章、`ch02-*` は第 2 章、以下同様です。
...
| `docs/ch13-open-source-llm` | 第 13 章 | オープンソース LLM のデプロイと微調整 |
...
旧式のフェーズディレクトリや、アルファベットの接尾辞が付いたフェーズディレクトリは、今後追加しません。章、画像、スクリプト設定を追加するときは、`ch01-*` から `ch13-*` までの番号体系を優先して使います。
...
この付録は、ページ番号、ディレクトリ名、本文見出しを混同せず、1 つの新しいページを正しい番号体系で説明できれば合格です。
