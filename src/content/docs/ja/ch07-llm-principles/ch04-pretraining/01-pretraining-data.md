...
### まずは物語で考える：2つのモデル pipeline が違うデータを読んだら？

同じ architecture と training setup を持つ 2 つのモデル pipeline があるとします。

1つ目の pipeline は、質の高い技術文書、論文、検証済みの製品ガイド、編集された長文で学習します。2つ目の pipeline は、重複した広告、見出しだけのページ、転載サイト、混乱したコメントで学習します。十分な training step のあと、表現品質、事実の信頼性、問題を分析する癖は、かなり違っているはずです。

事前学習データがモデルに与える影響は、まさにこれです。architecture は処理の仕組み、計算資源は training budget、data は pipeline に流れ込む材料です。材料が違えば、最終的な能力の土台も変わります。
...
:::tip[図の読み方]
学習コーパスと評価セットは、必ずきれいに分離します。ベンチマークの問題、解答、または近い変種が学習に漏れると、モデルは答え方のパターンを見たことがあるだけで高得点に見える場合があります。それは本当の汎化ではありません。
:::
### 区別しておきたい用語

- **データ汚染**: テストデータ、解答、近重複サンプルが学習コーパスに入ることです。ベンチマークのパターンを暗記し、転移できる能力と誤解される危険があります。
- **評価リーク**: 評価セットの情報が学習やプロンプト設計に影響することです。スコアが本当の未知データ性能を表さなくなります。
- **ベンチマーク**: モデル比較に使う標準テストセットです。便利ですが、Web 規模のコーパスに混ざりやすい点に注意します。
- **n-gram / hash 検査**: テキスト断片や指紋で重なりを比較する方法です。完全重複や、よく似たサンプルを見つけやすくなります。
- **バージョン管理**: 各コーパス版のデータソース、フィルタ、ルールを記録することです。記録がないと、スコア変化の説明や訓練の再現が難しくなります。
...
3. コード向けモデルなら、コード、ドキュメント、issue の議論、API reference、テスト、デバッグ trace の比率を上げます。ただし、指示理解や説明のために自然言語も十分残します。配分は一般的な比率ではなく、作りたいプロダクトの挙動に合わせます。
4. 評価サンプルが学習に混ざると、モデルは答えや表面パターンを覚えてしまいます。そのスコアは未知タスクへの汎化ではなく、「見たことがある」ことを測っている可能性があります。
