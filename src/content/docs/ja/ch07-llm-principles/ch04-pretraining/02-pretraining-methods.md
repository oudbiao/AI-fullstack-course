...
### 例えるなら、training task が system behavior を決める

training objective は、モデルを繰り返し動かす test harness のようなものです。

- harness が token を隠し続ければ、モデルは復元を練習する
- harness が続きを求め続ければ、モデルは next-token prediction を練習する
- harness が入力と期待出力を渡し続ければ、モデルは input-to-output mapping を練習する
