...
| 手法 | 学習する場所 | 主なトレードオフ |
|---|---|---|
| Prompt Tuning | 入力 embedding の前 | パラメータは非常に少ないが、改造の強さは限られる |
| Prefix Tuning | 各層の注意機構の KV プレフィックス | soft prompt より表現力が強いが、実装は複雑 |
| Adapter | 層間の小さなボトルネックモジュール | タスク切り替えはしやすいが、推論時に少し計算が増える |
| IA3 | 活性のスケーリングベクトル | とても軽いが、複雑な変化への表現は弱め |
