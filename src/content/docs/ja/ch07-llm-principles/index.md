...
- **必修コア**: Tokenization、embedding、コンテキストウィンドウ、LLM API 呼び出し、Prompt テスト、構造化出力、基本的な安全確認です。RAG と Agent アプリケーションへ進む前の最小スキルになります。
- **任意の拡張**: Transformer 内部、事前学習の詳細、微調整、整合の歴史です。モデル挙動、コスト、適応方法に深い説明が必要なときに戻ります。
- **深掘り課題**: mini GPT-2 を手作りし、固定評価セットを保ち、Prompt/スキーマ/model 設定を1つだけ変え、失敗例を保存します。LLM 利用をデモからエンジニアリングループへ変えます。
