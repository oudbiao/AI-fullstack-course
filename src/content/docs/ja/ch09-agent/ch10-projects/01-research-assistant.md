...
- 検索の取りこぼし
  重要資料が候補に入りません。クエリが狭すぎないか、キーワードが合っているか、`top_k` が小さすぎないかを確認します。クエリ書き換え、ハイブリッド検索、候補拡大後の rerank で改善します。

- 読解不足
  ヒットした文書は正しいのに、重要段落を落とします。chunk size と context packing を確認します。parent-child retrieval や context assembly の調整で改善します。

- 要約の過度な一般化
  要約はそれっぽいのに、制約条件が消えます。prompt を直し、claim、condition、source を保つ出力にします。

- 引用不一致
  claim と source が一致しません。citation check を追加し、claim ごとに検証します。

- ループ呼び出し
  Agent が検索を止められません。最大 step 数を追加し、新しい情報がないときは停止します。
