...
|---|---|---|
| Fast pass | 追跡可能な Agent loop を1つ作り、unsafe tool を1つ block する | `agent_traces.jsonl` と短い trace explanation |
| Standard pass | 中核ルート 9.1 -> 9.2 -> 9.3 -> 9.4 -> 9.8 -> 9.10 | `tools_schema.md`、`safety_boundary.md`、eval cases、failure trace |
| Deep pass | 単一 Agent loop が動いてから、MCP、framework comparison、deployment readiness、multi-agent coordination のいずれかを追加 | workflow/RAG/function calling より Agent が安全または有用な理由を書く design memo |
