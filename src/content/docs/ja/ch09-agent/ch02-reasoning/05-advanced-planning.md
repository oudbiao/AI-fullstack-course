...
1. 例の ワーカー 数を `1` と `3` に変えて、タイムラインの違いを比べてみましょう。
...
3. 極端な並列化は、コンテキスト切り替え、マージ衝突、重複作業、品質管理の負荷を増やします。明確に独立していて責任範囲が分かれている作業だけを並列化します。
4. 良い依存グラフは、独立タスク、ブロッキングタスク、review gate、最終統合を分けて表します。
