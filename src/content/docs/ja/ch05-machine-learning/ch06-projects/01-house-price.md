...
### ステップ 2.1 最初の特徴量エンジニアリングで、なぜ控えめにするのか
...
### ステップ 5.1 残差分析は、最終スコア以上に「できる人かどうか」が出る
...
1. EDA は図を描くだけでは完了しません。分布、欠損値、相関、怪しい外れ値を記録し、それがモデリングにどう影響するかを書きます。
2. 新しい特徴量には、面積比や品質の集約のような住宅価格としての意味が必要です。未来情報や目的変数から作った特徴量は leakage として除外します。
3. 複数モデルの比較では、同じ split または同じ交差検証を使います。単純な baseline を残して初めて、改善の効果を正直に判断できます。
4. ハイパーパラメータ調整は baseline が動いてから行います。検証データまたは交差検証で調整し、テストセットは最後の確認だけに使います。
5. 残差分析では、どこで外しやすいかを説明します。たとえば高価格帯や特定地域で誤差が大きいなら、次の実験はそこから考えます。
