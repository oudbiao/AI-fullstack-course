...
![train/test 分割の実践ガードレール](/img/course/ch05-hands-on-data-split-lab-ja.webp)
...
![機械学習の証拠パイプライン](/img/course/ch05-hands-on-evidence-pipeline-ja.webp)
...
- **ベースライン**：最初に作る最も単純なモデル。最低限超えるべき基準を示します。
...
![ML ワークショップコード実行順序図](/img/course/ch05-hands-on-code-execution-sequence-ja.webp)
...
![ColumnTransformer と Pipeline の学習フロー](/img/course/ch05-hands-on-pipeline-training-flow-ja.webp)
...
![しきい値と指標の判断ラボ](/img/course/ch05-hands-on-threshold-decision-lab-ja.webp)
...
![エラーサンプルのバケット確認](/img/course/ch05-hands-on-error-bucket-review-ja.webp)
...
![ML ワークショップの漏洩とデバッグ閉ループ](/img/course/ch05-hands-on-debug-loop-ja.webp)
...
![機械学習ポートフォリオ証拠パック](/img/course/ch05-hands-on-portfolio-pack-ja.webp)

![ML ワークショップの実験再実行ループ](/img/course/ch05-hands-on-rerun-experiment-loop-ja.webp)
...
1. クリーンなフォルダから動く実行コマンドがあると、隠れた Notebook 状態に依存せず出力を再生成できます。
2. baseline 指標と実モデル指標は、同じ split と同じ指標を使って 1 つの比較表に置きます。
3. leakage check では、目的変数由来の列が特徴量に入っていないこと、前処理が訓練 workflow の中で fit されていることを明記します。
4. 指標やしきい値の説明はタスク目標と結びつけます。特に false negative と false positive のコストが違う場合は重要です。
5. エラーサンプルは次の実験につながるべきです。特徴量修正、ラベル確認、セグメント分析、しきい値調整などが候補です。
6. README が、何を試し、何が改善し、どこで失敗し、次に何をするかを説明できれば、ポートフォリオに近づきます。
