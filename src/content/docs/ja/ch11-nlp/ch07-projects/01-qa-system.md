---
title: "11.7.2 プロジェクト：スマートQAシステム"
...
STOP_TOKENS = set("はがをにのですますか何どうやってできいつまでとでどちら強い") | {"gpu", "python"}


def tokenize(text):
    return {
        token
        for token in re.findall(r"[a-z0-9]+|[\u3040-\u30ff\u3400-\u9fff]", text.lower())
...
```text
{'matched_question': '講座は購入後何日以内なら返金できますか？', 'answer': '講座購入後 7 日以内で、かつ学習進捗が 20% 未満であれば返金を申請できます。', 'score': 2}
{'matched_question': '修了証はどうやって取得しますか？', 'answer': '必修項目をすべて完了し、修了テストに合格すると、修了証を取得できます。', 'score': 4}
```

ここでのスコアは、共有された有効 token の数です。とても単純ですが、文字をそのまま比べるだけより安定しており、範囲外の質問は `0` 点にできます。

### この例が、ただの関数ではなくプロジェクトらしいのはなぜ？
...
### `matched_question` を見せる価値が高いのはなぜ？
...
下のコードを前のコードの下に追加して、同じファイルをもう一度実行します。
...
```text
{'query': '返金はいつまでできますか', 'matched_question': '講座は購入後何日以内なら返金できますか？', 'score': 2}
{'query': '修了証はどうやって取りますか', 'matched_question': '修了証はどうやって取得しますか？', 'score': 4}
```

ヒットした質問が間違っているなら、先に回答文を調整するのではなく、検索、tokenization、ナレッジベースの境界を見直します。
...
```text
{'answer': '現在のナレッジベースには十分関連する情報がありません。', 'matched_question': None, 'score': 0}
```

これが望ましい動作です。根拠になる知識が見つからないため、推測せずに拒答しています。
...
```python
eval_data = [
    ("返金期間はどのくらいですか", "講座購入後 7 日以内で、かつ学習進捗が 20% 未満であれば返金を申請できます。"),
    ("修了証はどうやって取得しますか", "必修項目をすべて完了し、修了テストに合格すると、修了証を取得できます。"),
    ("前の4段階にGPUは必要ですか", "前の4段階では GPU は不要で、普通のパソコンで学習できます。"),
]
...
この評価セットは小さいですが、閉ループは確認できます。通常の質問に正しく答え、言い換えでも正しい項目にヒットし、拒答は別途テストできます。
