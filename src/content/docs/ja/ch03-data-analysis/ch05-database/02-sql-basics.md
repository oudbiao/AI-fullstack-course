...
- フィルタ問題は `WHERE`、並べ替えと top-k は `ORDER BY ... LIMIT`、都市別平均は `GROUP BY city` と `AVG(...)` を使います。
- 注文レポートでは、注文がある顧客だけでよければ `JOIN`、注文がない顧客も残すなら `LEFT JOIN` を使います。`COALESCE` は null の合計金額を 0 と表示するのに便利です。
- 合計金額がしきい値を超えるような集計後の条件は、先に集計して `HAVING` を使います。集計結果に依存するため `WHERE` ではありません。
