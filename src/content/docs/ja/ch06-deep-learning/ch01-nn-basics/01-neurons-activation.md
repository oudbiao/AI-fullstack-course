...
3. 200 step ごとの loss は、学習が進んでいるか、途中で止まっているかを確認するための最小ログです。
4. 活性化関数を外すと、線形層を重ねても全体は線形変換のままです。XOR のような非線形境界を表しにくくなります。
5. 隠れ層を追加すると容量は増えますが、小さなデータでは必ず改善するとは限りません。final loss と安定性を両方見ます。
