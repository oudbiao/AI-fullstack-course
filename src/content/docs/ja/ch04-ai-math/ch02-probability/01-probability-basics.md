...
# サービス状態：正常(0.85) / 劣化(0.15)
service_state = rng.choice(['正常', '劣化'], n, p=[0.85, 0.15])

# アラート確率はサービス状態によって変わる
alert = np.where(
    service_state == '劣化',
    rng.choice(['アラート', 'なし'], n, p=[0.75, 0.25]),  # 劣化時は 75% がアラート
    rng.choice(['アラート', 'なし'], n, p=[0.05, 0.95])   # 正常時は 5% が誤検知
)

# pandas を追加せず、NumPy だけで同時確率表を計算する
state_labels = ['正常', '劣化']
alert_labels = ['アラート', 'なし']
joint = np.array([
...
print(f"\n周辺確率 P(劣化): {(service_state == '劣化').mean():.3f}")
print(f"周辺確率 P(アラート): {(alert == 'アラート').mean():.3f}")
```
...
| モデルルーティング | リクエストが強いモデルを必要とする確率 | 類似した難例が小さいモデルで失敗した確率 | このリクエストを上位モデルへ回す確率 |
...
- [4.2.3 確率分布：データの背後にある規則](/ja/ch04-ai-math/ch02-probability/02-distributions/)
