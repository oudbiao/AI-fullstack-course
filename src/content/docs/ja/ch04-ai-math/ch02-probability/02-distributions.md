...
離散分布は事象を数えるときに使います。ベルヌーイ（`p`）、二項（`n, p`）、ポアソン（`lambda`）です。連続分布は範囲や測定値を表すときに使います。均一（`a, b`）、正規（`mu, sigma`）、指数（`lambda`）です。

| 必要なこと | 使う分布 | NumPy 呼び出し |
|---|---|---|
| 二値ラベル、または1回の yes/no 試行 | ベルヌーイ | `rng.binomial(1, p)` |
| `n` 回の試行で成功した回数 | 二項 | `rng.binomial(n, p)` |
| 固定区間で起きるまれな事象の回数 | ポアソン | `rng.poisson(lam)` |
| 範囲内のランダムな値 | 均一 | `rng.uniform(a, b)` |
| ノイズや重みの初期化 | 正規 | `rng.normal(mu, sigma)` |
| 事象間の時間間隔 | 指数 | `rng.exponential(1/lam)` |
