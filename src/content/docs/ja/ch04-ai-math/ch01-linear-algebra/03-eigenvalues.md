...
| 用語 | やさしい意味 | なぜ重要か |
|---|---|---|
| `λ` / `lambda` | 固有ベクトルが伸び縮みする倍率 | `λ = 3` なら、その方向は変換後に 3 倍の長さになります。 |
| `np.linalg.eig` | NumPy で固有値と固有ベクトルを求める関数 | 先に固有値、次に固有ベクトルを返します。 |
| `eigenvectors[:, i]` | i 番目の固有ベクトル | NumPy では固有ベクトルが行ではなく列として保存されます。見落としやすい点です。 |
| 共分散行列 | 特徴量どうしがどのように一緒に変化するかを表す行列 | PCA はこの行列から重要な方向を見つけます。 |
| `eigvalsh` | 対称/エルミート行列向けの固有値計算関数 | 共分散行列は対称なので、固有値だけならこちらが速く安定しやすいです。 |
| PCA | Principal Component Analysis、主成分分析 | 変化が大きい方向を残す次元削減の方法です。 |

数式を読むときは、まず「固有ベクトルは方向、固有値はその方向の強さ」と考えましょう。
...
環境によっては、固有ベクトル全体の符号が反転することがあります。たとえば `[0.707, -0.707]` と `[-0.707, 0.707]` は同じ方向の直線を表すので、エラーではありません。
...
この確認は、元の数値そのものより重要です。`A @ v` と `λ * v` がほぼ等しければ、その組は本当に固有ベクトルの定義を満たしています。
...
これは、第1主方向がこの小さなデータセットの変化の約 94% を保っているという意味です。つまり 2D から 1D に減らしても、情報の損失は比較的小さいと考えられます。
