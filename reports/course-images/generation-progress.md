...
Status: website-safe, English/Japanese localized placeholders completed.

Generated with image2: 618 original zh assets + 15 English homepage comics.

Local localized placeholders: 1191 / 1191
...
- 1191 localized placeholder images still need final image2 replacement.
- Full pending filename list: `reports/course-images/localized-placeholder-pending.txt`.
- English/Japanese docs already point to these localized files, so the website is safe while replacements are generated gradually.

## Resume Command

```bash
# Rebuild localized placeholders from current i18n docs and source image sizes.
python3 scripts/localize_course_image_placeholders.py --overwrite
```

```bash
# Re-run the image localization audit after replacing any batch with image2 outputs.
