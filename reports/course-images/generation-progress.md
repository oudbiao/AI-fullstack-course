...
Status: website-safe; current site image references resolve, and no confirmed fallback/placeholder course image assets remain in active references.

Active reference audit: 2215 `/img/course` assets referenced by docs/i18n/src content, with 0 missing files. Of those, 2116 are PNG image assets and 99 are code-generated SVG teaching diagrams.

Localized pending list: 0 files in `reports/course-images/localized-placeholder-pending.txt`.

Detection note: earlier report text about 1191 localized placeholder PNGs is stale. The current pending list is empty, and `localized-generation-progress.jsonl` has no filename whose latest status is still `error`.
...
Sitemap QA pending: after all internationalized content and images are finalized, run a production build and verify `build/sitemap.xml` includes the new English and Japanese pages, including locale-prefixed URLs. Confirm the deployment pipeline regenerates sitemap output rather than serving a stale sitemap.

英日图片占位说明：旧报告中关于英日 localized placeholder 的说明已经过期；当前站点实际引用的课程 PNG 没有 1x1 fallback、没有小尺寸失败图，也没有文件名含 placeholder/pending 的引用资产。
...
首页漫画书说明：本轮将根目录首页 `docs/index.md` 大改为“人工智能发展史漫画书”，新增 15 张分阶段漫画页，覆盖图灵、达特茅斯会议、感知机、专家系统、反向传播、LeNet/CNN、统计机器学习、ImageNet/AlexNet、ResNet、RNN/LSTM、Attention、Transformer、BERT/GPT、SFT/RLHF/ChatGPT、RAG/工具调用/Agent。15 张图片已经全部使用 image2 重新生成，并统一升级为 `1024x1792` 竖版故事漫画；中文标题、对话气泡、小黑板、成功点、问题、下一阶段和历史意义直接融入漫画画面，不再使用本地大块文字叠加版。

英文国际化说明：本轮开启 Docusaurus `en` locale，新增 `/en/` 英文首页、英文导航/页脚/侧边栏翻译文件，并为首页 AI 发展史漫画书生成 15 张英文版 image2 竖版故事漫画，文件名统一为 `homepage-ai-history-comic-en-*.png`。英文图片中的标题、气泡、小黑板、成功点、风险和历史意义直接融入画面，避免英文页面继续显示中文漫画。
...
- `homepage-ai-history-comic-01-turing.png`
- `homepage-ai-history-comic-02-dartmouth.png`
- `homepage-ai-history-comic-03-perceptron.png`
- `homepage-ai-history-comic-04-expert-systems.png`
- `homepage-ai-history-comic-05-backprop.png`
- `homepage-ai-history-comic-06-lenet.png`
- `homepage-ai-history-comic-07-statistical-ml.png`
- `homepage-ai-history-comic-08-imagenet-alexnet.png`
- `homepage-ai-history-comic-09-resnet.png`
- `homepage-ai-history-comic-10-rnn-lstm.png`
- `homepage-ai-history-comic-11-attention.png`
- `homepage-ai-history-comic-12-transformer.png`
- `homepage-ai-history-comic-13-bert-gpt.png`
- `homepage-ai-history-comic-14-rlhf-chatgpt.png`
- `homepage-ai-history-comic-15-rag-agent.png`
...
- `appendix-debug-mre-help-flow.png`
- `appendix-hardware-local-cloud-api-cost-map.png`
- `appendix-job-portfolio-storyline-map.png`
- `appendix-learning-paper-project-notes-loop.png`
- `appendix-resource-bottleneck-priority-map.png`
- `appendix-faq-confidence-reset-map.png`
- `appendix-quick-ref-debug-index-map.png`
...
- No confirmed active placeholder PNG/SVG assets found in the current `/img/course` references.
- Keep `reports/course-images/localized-placeholder-pending.txt` empty unless a future audit intentionally re-adds files for regeneration.
- Historical API failures in `localized-generation-progress.jsonl` have later successful `generated` records for the same filenames.

## Verification Commands

```bash
# Re-run the image localization audit after replacing any future image batch.
python3 scripts/audit_image_i18n.py --skip-ocr
```

```bash
# Check active /img/course references for missing files and obvious fallback placeholders.
python3 - <<'PY'
from pathlib import Path
import re, struct
root = Path.cwd()
files = []
for base in ["docs", "i18n", "src", "blog"]:
    path = root / base
    if path.exists():
        files += [p for p in path.rglob("*") if p.suffix.lower() in {".md", ".mdx", ".js", ".jsx", ".ts", ".tsx", ".html"}]
image_re = re.compile(r"/img/course/([A-Za-z0-9_.@%+=,;:~!$&*()\[\]\-]+\.(?:png|svg|jpg|jpeg|webp))")
refs = {}
for path in files:
    ...
    for match in image_re.finditer(text):
        refs.setdefault(match.group(1), []).append(str(path.relative_to(root)))
static = root / "static" / "img" / "course"
missing = sorted(name for name in refs if not (static / name).exists())
tiny_png = []
svg_placeholders = []
for name in refs:
    path = static / name
    ...
    if path.suffix.lower() == ".png":
        ...
        if path.stat().st_size < 5000 or (width, height) == (1, 1):
            tiny_png.append((name, path.stat().st_size, width, height))
    elif path.suffix.lower() == ".svg":
        head = data[:3000].decode("utf-8", errors="ignore").lower()
        if "placeholder" in head:
            svg_placeholders.append(name)
print({"refs": len(refs), "missing": missing, "tiny_png": tiny_png, "svg_placeholders": svg_placeholders})
PY
```
