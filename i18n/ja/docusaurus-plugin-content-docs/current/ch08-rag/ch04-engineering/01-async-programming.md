...
## 一、なぜ LLM エンジニアリングは特に「待ち」が起きやすいのか？
...
### 1.2 重要な点：多くの処理は「計算が遅い」のではなく「待ちが遅い」
...
## 四、なぜ `gather` はこんなに使われるのか？

### 4.1 LLM の場面では「複数の路線を同時に調べる」ことが多いから
...
![非同期並行、Semaphore と timeout 制御の図](/img/course/ch08-async-concurrency-semaphore-timeout-map.png)
...
## 六、タイムアウト制御がなぜ特に重要なのか？

### 6.1 一部のリクエストは「固まる」ことがあるから
...
### 7.5 初めて非同期をプロジェクトに入れるときの、もっとも安全な順番
...
## 八、目標が「ナレッジベース駆動の教材生成アシスタント」なら、どの処理を並行化するべきか？
