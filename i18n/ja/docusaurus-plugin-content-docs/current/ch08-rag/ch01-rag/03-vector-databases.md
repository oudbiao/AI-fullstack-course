...
## 五、目標が「知識ベース駆動の SOP 文書アシスタント」なら、メタデータには最低でも何を持たせるべきか？
...
| `content_type` | ポリシー / 対応ケース / チェックリストを区別する |
...
record = {
    "id": "doc_001_chunk_03",
    "text": "返金期限後でも重複請求が確認された場合は、取引証拠を添えて billing support にエスカレーションする。",
    "metadata": {
        "topic": "返金エスカレーション",
        "content_type": "case",
...
- ベクトルストアのこの層が、後の SOP 文書を安定して組み立てられるかどうかを実は左右している
