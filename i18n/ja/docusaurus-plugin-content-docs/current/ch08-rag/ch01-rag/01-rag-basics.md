...
|---|---|---|
| 知識ベースに答えがあるのに命中しない | chunk が大きすぎる、キーワードが一致しない、embedding が適していない | top-k を出力し、クエリ と chunk の文面を確認する |
| 正しい文書には命中したが、答えから重要条件が抜ける | chunk が不完全、コンテキスト の順序が不自然、prompt の制約が弱い | overlap を増やす、コンテキスト構成 を調整する、条件を引用するよう指示する |
