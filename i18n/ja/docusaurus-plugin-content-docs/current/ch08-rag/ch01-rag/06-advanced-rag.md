...
### 1.1 基本の RAG は「1回の質問 -> 1回の検索 -> 1回の回答」に向いている
...
### 2.1 1つのナレッジベースで足りないなら、まずルーティングする
...
### 8.1 複雑なタスクを見るとすぐ Agentic RAG にしたくなる

多くの場合、まずルーティングと検索戦略を整えるだけで、大半の問題は解決できます。

### 8.2 「高度」を「コンポーネントが多いほど高度」と考える

コンポーネントが増えても、必ずしもシステムがよくなるわけではありません。  
ただ保守が難しくなるだけかもしれません。

### 8.3 評価せずにアーキテクチャをむやみにアップグレードする
