...
## 1. なぜマルチターン対話は単一ターンQ&Aよりずっと難しいのか？

### 1.1 単一ターンQ&Aは「1問1答」に近い
...
### 3.1 このコードは小さいですが、すでに何を教えているのでしょうか？
...
![対話状態、スロット、記憶管理の図](/img/course/ch08-dialog-state-slot-memory-map.png)
...
## 4. 対話システムは回答するだけでなく、確認質問もできる必要がある
...
### 4.3 なぜ確認質問は「システムが安定している」ことの表れなのか？
...
## 5. なぜ「履歴を全部そのままモデルに入れる」だけでは足りないのか？
...
### 6.1 この例は、普通のQ&Aと比べて何が増えたのでしょうか？
...
## 8. もし目標が「知識ベース駆動の教材生成アシスタント」なら、どのスロットを管理すべきか？
...
## 10. 初学者が最初に対話システムを作るときの、いちばん安全な順番
...
## 11. これをプロジェクトとして見せるなら、何を見せるのが一番よいか
...
### 13.1 history だけを管理して、構造化状態を管理しない

システムの制御がどんどん難しくなります。

### 13.2 1回でわからないと、すぐ推測する
