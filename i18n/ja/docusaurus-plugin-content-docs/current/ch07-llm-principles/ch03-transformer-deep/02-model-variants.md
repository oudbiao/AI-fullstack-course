...
### 2.1 Encoder-only：理解に向いていて、自由な続きを書くのは得意ではない
...
### 2.3 Encoder-Decoder：入力と出力の役割を分ける
...
### 2.4 MoE：情報の流れを変えるのではなく、「誰が計算するか」を変える
...
![アーキテクチャの mask とタスク適合図](/img/course/ch07-architecture-mask-task-fit-map.png)
...
### 4.1 テキスト理解タスクで Encoder-only がよく使われるのはなぜ？
...
### 4.2 なぜ今の大規模モデルは、ほぼ Decoder-only が主流なのか？
...
### 4.3 Encoder-Decoder が消えていないのはなぜ？
...
### 5.2 「性能差」の多くは、構造だけでなく学習規模とデータから来る
...
### 6.1 誤解1：BERT は「古いモデル」だから、学ぶ必要はない

違います。  
今でも、理解タスクや表現学習の重要な基準モデルです。

### 6.2 誤解2：Decoder-only は何でもできるから、必ず最適解

たしかに汎用性は高いです。  
ただし、入力と出力がはっきり分かれたタスクでは、  
encoder-decoder のほうが自然なこともあります。

### 6.3 誤解3：MoE は「ただのもっと大きい普通のモデル」
