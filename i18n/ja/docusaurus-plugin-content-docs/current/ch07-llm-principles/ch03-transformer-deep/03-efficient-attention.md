...
### 1.1 各 token はたくさんの token と比較する必要がある
...
### 2.1 スライディングウィンドウ / ローカル注意力：誰が誰を見るかを減らす
...
### 2.3 FlashAttention：公式ではなく、計算の仕方を変える
...
### 3.2 なぜ `full pairs` と `local pairs` は大きく違うのか？
...
### 5.1 ボトルネックは「計算できないこと」ではなく「データ移動が高すぎること」だから
...
### 7.2 誤解2：長いコンテキストに対応すれば、モデルは必ずそれを使いこなせる
...
### 7.3 誤解3：FlashAttention は新しいモデルアーキテクチャである
