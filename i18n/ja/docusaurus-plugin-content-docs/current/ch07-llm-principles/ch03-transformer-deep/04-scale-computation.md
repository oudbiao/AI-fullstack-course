...
### 1.2 同じ大規模モデルでも、コストは他の多くの要素で変わる
...
### 2.1 decoder block の主な重みは Attention と FFN
...
### 3.2 なぜ `large` モデルの負荷は単純な倍増ではないのか？
...
### 4.3 なぜ「学習できる」ことと「うまくデプロイできる」ことは別なのか？
...
### 5.1 パラメータを増やすのは、能力の可能性を増やすことであって、無料で性能が上がるわけではない
...
### 7.1 誤解1：パラメータが大きければ、必ず性能も良い

不十分です。  
パラメータ数は容量であって、自動的に性能を保証するものではありません。

### 7.2 誤解2：推論コストはパラメータ数だけで決まる

違います。  
コンテキスト長と KV cache も、同じくらい重要になることがよくあります。

### 7.3 誤解3：学習用メモリと推論用メモリは同じ
