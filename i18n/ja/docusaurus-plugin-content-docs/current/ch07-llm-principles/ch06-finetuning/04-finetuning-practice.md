...
### 1.2 まず baseline を作ってから、微調整を考える
...
### 2.3 loss が下がることは、業務で使えることと同義ではない
...
### 4.1 SFT では、通常 assistant 部分だけを学習対象にしたい
...
### 4.2 形式ルールが不安定だと、モデルは「汚いパターン」を覚える
...
### 5.1 なぜ effective batch size を特に気にするのか？
...
### 5.2 なぜ検証時に `val_loss` だけ見てはいけないのか？
...
### 6.3 第 3 層: 過学習や catastrophic forgetting が起きていないか
...
### 7.1 オフライン評価に通っても、そのまま本番投入してはいけない
...
### 7.3 微調整プロジェクトは「1 回の学習」ではなく、継続的な反復の流れである
...
### 8.1 誤解 1: いきなり学習パラメータを決める

先にパラメータを決めるのは、もっとも重要なタスク定義とデータ整理を飛ばしてしまうことが多いです。

### 8.2 誤解 2: データは多ければ多いほどよい
