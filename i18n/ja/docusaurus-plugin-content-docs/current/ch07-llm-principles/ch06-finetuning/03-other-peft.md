...
### 1.1 PEFT が本当に解決したいのは「略語を発明すること」ではない
...
### 2.1 Prompt Tuning：学習可能な部分を入力の前に置く
...
### 2.2 Prefix Tuning：各層に「プレフィックス文脈」を加える
...
### 2.3 Adapter：層と層の間に小さなモジュールを入れる
...
### 2.4 IA3：大きな行列は学ばず、「スケーリング係数」だけを学ぶ
...
![PEFT 方法の学習可能パラメータ配置図](/img/course/ch07-peft-placement-family-map-ja.png)
...
### 4.1 Prompt Tuning：入力の前にソフトプロンプトを連結する
...
### 4.2 Prefix Tuning：入力長ではなく、各層の注意文脈を変える
...
### 4.3 IA3：モジュールを足すのではなく、重要チャネルにスケールを掛ける
...
### 6.2 誤解 2：手法名をたくさん知っていれば理解できている
