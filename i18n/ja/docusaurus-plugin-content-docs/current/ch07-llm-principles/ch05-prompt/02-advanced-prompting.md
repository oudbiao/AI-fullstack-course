...
| 用語 | 意味 | いつ使うか |
|---|---|---|
| Zero-shot | 例を出さずに、タスクだけを直接渡す方法 | タスクが単純で、ラベル境界が明確なときにまず試す |
| Few-shot | 本番入力の前に、少数の入力と出力の例を見せる方法 | 定義だけでは足りず、モデルに判断例を真似してほしいとき |
...
### 3.3 「役割はタスク定義の代わりにならない」ことがわかる最小例
