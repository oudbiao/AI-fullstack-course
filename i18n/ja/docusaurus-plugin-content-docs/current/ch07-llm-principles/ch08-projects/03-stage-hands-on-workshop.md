...
![第 7 章実践ワークショップのルート図](/img/course/ch07-hands-on-workshop-route-ja.png)

これがファイルの中でたどる道筋です。tokens、payload、Prompt 版、検証、手法判断、証拠へ進みます。

![Prompt payload と検証ループ図](/img/course/ch07-hands-on-payload-validation-loop-ja.png)

モデル風の回答は、プログラムが信頼する前に parser、field check、type check を通ります。

![第 7 章ワークショップのコード実行順序図](/img/course/ch07-hands-on-code-execution-trace-ja.png)

スクリプトは、まず痕跡を作り、次に Prompt を評価し、手法ルートを選び、最後にファイルを保存します。

![Prompt、RAG、ファインチューニングのルート階段図](/img/course/ch07-hands-on-route-decision-ladder-ja.png)
