...
- この prompt は コンテキストウィンドウ を無駄遣い、または超過していないか。
:::
...
| `padding_side` | padding を左に足すか右に足すか。デコーダーモデルでは重要なことがある |
| コンテキスト長 | 入力と生成出力が共有する最大 token 予算 |
...
- デコーダーモデルを batch 化するときは padding 方向と `attention_mask` を確認する。
...
```text
サンプル文: 1つの英語/CJK/コード風の例
トークン：印字した token 一覧と token 数
切り捨てケース：何が切り捨てられ、なぜ重要か
製品リスク：コスト、コンテキスト制限、または指示の欠落
デバッグ行動: モデルを疑う前にトークナイゼーションを確認する
```
