...
### 1.1 それは、モデルが「学習中に何度も求められたこと」を優先して覚えるから
...
### 2.1 Causal Language Modeling: 過去から未来を予測する
...
### 2.2 Masked Language Modeling: 文脈から空欄を埋める
...
### 2.3 Span Corruption / Denoising: 1語ではなく、1区間を隠す
...
### 3.2 なぜ Causal LM のラベルは1つ右にずれるのか？
...
### 3.3 なぜ Span Corruption はより「汎用的」と見なされやすいのか？
...
### 4.3 Span Corruption: 入力から出力への変換
...
### 5.1 なぜ Decoder-only には Causal LM がよく合うのか？
...
### 5.2 なぜ Encoder-only には Masked LM がよく合うのか？
...
### 5.3 なぜ Encoder-Decoder には denoising 系がよく合うのか？
...
### 6.3 自己教師あり目標でも、完全に「偏りがない」わけではない
...
### 7.1 誤解1: 事前学習目標は前半の細かい設定にすぎず、後の fine-tuning ですべて解決できる

違います。  
事前学習目標は、モデルに長期的な能力の偏りを与えます。

### 7.2 誤解2: Masked LM は Causal LM より高級、またはその逆

どちらが上という関係ではありません。  
それぞれ別の方向に合わせた設計です。

### 7.3 誤解3: 名前だけ覚えて、ラベルの形を見ない
