...
### 因果言語モデリング（Causal Language Modeling）：過去から未来を予測する
...
### マスク言語モデリング（Masked Language Modeling）：文脈から空欄を埋める
...
### スパン破壊とノイズ除去（Span Corruption / Denoising）：1語ではなく、1区間を隠す
...
### なぜ因果言語モデル（Causal LM）のラベルは1つ右にずれるのか？
...
### なぜスパン破壊（Span Corruption）はより「汎用的」と見なされやすいのか？
...
### スパン破壊（Span Corruption）：入力から出力への変換
...
### なぜデコーダーのみには因果言語モデル（Causal LM）がよく合うのか？
...
### なぜエンコーダーのみにはマスク言語モデル（Masked LM）がよく合うのか？
...
### プレフィックス言語モデル（Prefix LM）：一部は双方向、一部は因果的
...
### 誤解2：マスク言語モデル（Masked LM）は因果言語モデル（Causal LM）より高級、またはその逆
