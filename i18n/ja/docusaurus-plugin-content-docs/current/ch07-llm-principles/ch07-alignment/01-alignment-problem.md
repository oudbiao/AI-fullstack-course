...
### 有用性（Helpful）：助けるべき場面では、ちゃんと役に立つ
...
### 誠実性（Honest）：分からないなら分からないと言う。知ったかぶりしない
...
### 無害性（Harmless）：やってはいけないことは、きちんと止める
...
| 観点 | 何を問いかけているか |
|---|---|
| 有用性（Helpful） | この答えは本当にユーザーの役に立っているか？ |
| 誠実性（Honest） | 不確かなときに、境界を正直に保てているか？ |
| 無害性（Harmless） | 安全やコンプライアンスの境界を越えていないか？ |

この表は、最初にしっかり覚える価値があります。後で出てくる RLHF、ルールベースのガードレール、評価 rubric などは、結局この3種類の問題を中心に考えることになるからです。

![有用性、誠実性、無害性の対立図](/img/course/ch07-alignment-hhh-tension-guardrail-map-ja.webp)

:::tip 図の読み方
この図では、3つの力が引っ張り合っていることに注目してください。有用性は役に立つこと、誠実性は境界を認めること、無害性はリスクを止めることです。アライメントは、ただ拒否することでも、何でも助けることでもありません。異なる依頼ごとに、この3つのバランスを取り、評価・戦略・ガードレールで実装していくことです。
:::
...
|---|---|---|
| HHH | 有用性・誠実性・無害性（Helpful、Honest、Harmless） | アライメントの 3 つの中心目標を覚えるための言葉 |
...
2. 自分がよく知っている業務シーンを1つ選び、有用性、誠実性、無害性それぞれの判断ルールを1つずつ書いてみましょう。
...
3. よい答えでは、単に「悪い回答」と言うのではなく、どの観点で失点したかを示します。たとえば、自信満々だが誤った返金ルールは誠実性で失点し、正しいが他人の顧客情報を漏らす回答は無害性で失点します。
4. Tool 接続システムでは、権限外の読み書き、危険な tool 実行、取得データからの hidden prompt injection、広すぎる権限、取り消せない操作などのリスクが増えます。
