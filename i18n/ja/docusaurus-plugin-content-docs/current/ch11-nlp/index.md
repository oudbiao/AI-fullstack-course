---
title: "11 自然言語処理（方向選択）"
sidebar_position: 0
description: "生テキストから token、表現、分類、抽出、生成、評価、事前学習モデルへ進む実践ルートで NLP を学ぶ。"
...
この選択章で答える問いは、**生のテキストが、どうやってモデルが分類・抽出・検索・生成できる対象になるのか**です。LLM は多くの NLP 手順を包み込みましたが、Prompt、RAG、Agent memory、検索、評価、情報抽出は今も NLP の考え方に支えられています。

## 11.0.1 まずテキストからタスクへの流れを見る

![テキストから NLP タスクへのパイプライン](/img/course/ch11-text-to-task-pipeline-ja.svg)
...
## 11.0.4 出力で NLP タスクを選ぶ

![NLP タスク出力図](/img/course/ch11-nlp-task-output-map-ja.svg)
