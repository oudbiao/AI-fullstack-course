...
![AI コンプライアンスのエンジニアリング変換図](/img/course/ch12-ai-regulation-engineering-translation-map-ja.webp)
...
3. risk classification は system design を変えます。低リスク機能なら軽い disclosure で済む場合がありますが、高リスク機能ではより厳しい logging、access control、review、fallback、human approval が必要になります。
4. 実装可能な要件は、prompt/model/version logs を保存すること、sensitive operations に permission check を入れること、危険な生成物は公開前に manual approval を必須にすることです。
