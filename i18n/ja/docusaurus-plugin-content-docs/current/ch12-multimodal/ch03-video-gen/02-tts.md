...
![TTS テキストから音声への生成パイプライン図](/img/course/ch12-tts-text-to-speech-pipeline-map.png)
...
## 3. なぜテキスト処理の段階を省けないのか？

### 3.1 文字そのものは発音情報と同じではないから
...
## 5. Vocoder（声碼器）は何をしているのか？

### 5.1 役割は「スペクトルを本当に聞ける音に翻訳する」ことに近い
...
### 10.1 TTS は「文字を読むだけ」だと思ってしまう

実際には、自然な発話の過程を生成するものに近いです。

### 10.2 音色だけを見て、リズムや間を見ない

「不自然さ」の原因は、音色そのものではなく、韻律にあることが多いです。

### 10.3 TTS は最初からリアルタイムだと思ってしまう
