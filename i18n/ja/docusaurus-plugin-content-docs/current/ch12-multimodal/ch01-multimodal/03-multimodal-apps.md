...
### 「かっこいいから画像を足す」のではなく、入力が本当により完全になる
...
### なぜ多くのマルチモーダルアプリは「1つのモデルですべて完結」しないのか？
...
### 多くの「画像を見る問題」は、実は「文字を読む問題」でもあるから
...
![マルチモーダルのフォールバック結果図](/img/course/ch12-multimodal-fallback-no-text-result-map-ja.webp)
...
### マルチモーダルアプリは、最初から最も複雑なモデルを使うべきだと思い込む
...
### 画像が見られれば、システムは「場面を理解している」と思い込む
...
1. `404 Not Found` の分岐では、まず欠けているものがルートなのかリソースなのかを見分けます。そのうえで、URL パス、サーバー側ルート登録、デプロイ時の rewrite 設定、バックエンドが起動しているかを確認するよう促すと実用的です。
2. `material` 属性は、画像から十分に判断できる場合だけ追加します。たとえば `cotton`、`leather`、`metal` は商品マッチングに役立ちますが、低解像度や強いスタイル加工がある場合は不確実として扱います。
3. プロンプトは「画像が不鮮明です」だけで終わらせず、足りない情報を具体的に 1、2 個求めます。例として「完全なエラーメッセージとブラウザのアドレスバーが入る、より鮮明なスクリーンショットをアップロードしてください」と返すと、ユーザーが次に動きやすくなります。
