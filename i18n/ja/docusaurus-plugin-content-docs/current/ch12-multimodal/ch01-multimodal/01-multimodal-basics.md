...
![マルチモーダル特徴融合の実行結果図](/img/course/ch12-multimodal-feature-fusion-result-map-ja.webp)
...
3. スクリーンショットには、レイアウト、色、エラー位置、ボタン状態、視覚的な文脈が含まれます。文字だけではそれらが失われます。マルチモーダルシステムなら、視覚手がかりとユーザーの質問を合わせて、何が問題で次にどこを見るべきかを答えられます。
