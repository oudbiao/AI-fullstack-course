...
3. GridSearchCV では preprocessing と model parameters を同時に調整します。次元削減や特徴選択が、最適なモデル設定を変えることがあるためです。
4. 最終成果には fitted pipeline、CV スコア、選ばれたパラメータ、preprocessing が pipeline 内で行われていることを示す leakage check を残します。
