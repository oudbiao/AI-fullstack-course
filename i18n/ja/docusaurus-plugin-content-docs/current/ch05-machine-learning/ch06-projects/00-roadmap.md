...
このプロジェクトの流れがまだ抽象的に感じる場合は、先に [実践ワークショップ：再現可能な ML 証拠パックを作る](./05-hands-on-ml-workshop.md) を実行してください。住宅価格、顧客離反、ユーザーセグメンテーション、Kaggle に進む前に、完全に動くプロジェクト練習を 1 回行えます。
:::
