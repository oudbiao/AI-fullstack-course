...
- **ベースライン**：最初に作るシンプルな比較用モデルです。ベースラインがないと、後の改善が本当に効いたか判断できません。
...
- ベースライン、交差検証、特徴量エンジニアリング、提出記録をひとつの流れとしてつなげる
...
![Kaggle のローカル検証とリーダーボードのループ](/img/course/ch05-kaggle-validation-leaderboard-loop-ja.webp)
...
1. Titanic の 0.80+ は挑戦目標です。本当の学習目標は、再現可能な baseline、特徴量の反復、検証記録、提出ログを作ることです。
2. `Pipeline` または同等の再現可能な workflow を使い、前処理を一貫して適用します。テストラベルや leaderboard の反応を検証データのように扱ってはいけません。
3. 提出ごとに大きな変更は 1 つだけにし、変更内容、ローカル検証スコア、公開スコア、自分の解釈を記録します。
4. House Prices では、複雑な ensemble より先に、欠損値処理、カテゴリエンコード、交差検証を優先します。
5. 優れた Notebook を読む価値はありますが、そのアイデアがなぜ効くかを説明し、自分の pipeline に作り替えられることが条件です。
