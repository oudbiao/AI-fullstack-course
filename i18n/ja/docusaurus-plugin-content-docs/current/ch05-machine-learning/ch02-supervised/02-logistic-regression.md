...
![ロジスティック回帰の決定境界図](/img/course/logistic-regression-boundary.png)
...
![ロジスティック回帰の分類主線図](/img/course/ch05-logistic-classification-flow.png)
...
3. 最後に、sklearn の `multinomial` などの実装に戻る
...
# 多クラスロジスティック回帰
# multi_class='multinomial' は Softmax を使用
# multi_class='ovr' は One-vs-Rest を使用
...
for i, p in enumerate(proba):
    ...
    print(f"  サンプル {i}: {dict(zip(iris.target_names, np.round(p, 3)))} "
...
axes[0].set_title(f'L1 正則化（非ゼロのパラメータ: {np.sum(model_l1.coef_[0] != 0)}/20）')
axes[0].set_xlabel('特徴番号')
axes[0].set_ylabel('|パラメータ値|')

axes[1].bar(range(20), np.abs(model_l2.coef_[0]), color='steelblue')
axes[1].set_title(f'L2 正則化（非ゼロのパラメータ: {np.sum(model_l2.coef_[0] != 0)}/20）')
...
![ロジスティック回帰のしきい値トレードオフ図](/img/course/ch05-logistic-threshold-tradeoff.png)
