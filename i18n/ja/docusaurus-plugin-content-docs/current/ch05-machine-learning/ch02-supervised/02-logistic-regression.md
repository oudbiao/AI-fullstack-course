...
![OvR と Softmax の多クラスロジスティック回帰の比較図](/img/course/ch05-logistic-multiclass-ovr-softmax-ja.png)

実装に入る前に、まずこの図で 2 つの考え方を分けてください。OvR は K クラス問題を K 個の「このクラスか？」という二値分類器に分けるため、複数のスコアの合計が 1 になるとは限りません。Softmax は 1 つの正規化された確率分布を出すので、クラス間の確率を直接比較できます。初心者のプロジェクトでは、まず下の sklearn pipeline から始め、明示的に OvR 戦略を使いたいときだけ OvR ラッパーを使うのが安全です。

### One-vs-Rest（OvR）

K 個のクラスに対して K 個の二値分類器を学習し、それぞれが「このクラスか、それ以外か」を判定します。

### 初心者が多クラス分類を最初に理解するときのおすすめ順序
...
![ロジスティック回帰の正則化概念図](/img/course/ch05-logistic-regularization-concepts-ja.png)

パラメータを調整する前に、この図を読んでください。`C` は重みをどれくらい自由に大きくできるかを制御しますが、正則化強度の逆数です。つまり、`C` が小さいほど正則化は強くなります。`L2` は多くの場合すべての係数を小さくし、安定した基本選択です。`L1` は一部の係数を 0 にできるため、疎で解釈しやすいモデルがほしいときに役立ちます。`l1_ratio` はこの 2 つの性質を混ぜるためのものです。
...
### 初めて分類プロジェクトをするとき、なぜロジスティック回帰を最初に試す価値が高いのか？
