...
| 指標（Metrics） | 集約された傾向 | success_rate、latency_ms、cost、tool_error_rate |
| 追跡記録（Trace） | リクエストの流れ | request_id、step_id、node、input、output、status |
| 再現用記録（Replay） | 失敗の再現 | 元の入力、検索結果、ツールの返答、モデル設定、最終出力 |
