...
| 部分 | やさしい意味 | 必ず制御すること |
|---|---|---|
| 目標（Goal） | Agent が達成したいこと | 範囲、成功基準、停止条件 |
| 状態（State） | 今分かっていること | 現在の入力、過去の観察、残り手順 |
| 計画（Plan） | 次に試すこと | 最大ステップ、代替経路、人への引き継ぎ |
| ツール（Tool） | 検索、ファイル読み取り、API 呼び出し、コード実行など | schema、検証、ホワイトリスト、リスクレベル |
| 観察 | ツールが返した結果 | エラー処理、再試行ルール、信頼境界 |
| 記憶（Memory） | ステップ間・実行間で残す情報 | 短期状態と長期好みの境界 |
| 追跡記録（Trace） | 実行を再生できる記録 | 目標、行動、引数、観察、コスト、最終結果 |
...
| 層 | いま学ぶこと | どう使うか |
|---|---|---|
| 必修コア | 単一 Agent ループ、tool schema、ホワイトリスト、最大ステップ、state 境界、memory 境界、trace ログ、安全ブロック、評価ケース | 単なるデモではなく、レビューできる Agent の最小スキルです |
| 任意の拡張 | MCP、フレームワーク比較、多 Agent 協調、配置運用、コスト最適化 | 単一 Agent ループが安定し、統合や規模が必要になったときに戻ります |
| 深掘り課題 | 同じタスクを workflow、RAG flow、Function Calling、Agent trace として比較し、最も単純で安全な設計を説明する | 流行ではなく意図を持って Agent を使うためです |
...
| 実務準備 | tool schema を定義し、ホワイトリスト外のツールをブロックし、最大ステップを設定し、失敗追跡を保存できる。 |
...
| 1回の構造化された行動で十分 | Function Calling | 複数のツール呼び出しと状態更新が必要 |
