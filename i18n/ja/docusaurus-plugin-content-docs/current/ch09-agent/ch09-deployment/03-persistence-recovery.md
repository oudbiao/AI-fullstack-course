...
## 一、なぜ Agent には特に復元能力が必要なのか？

### 1.1 なぜなら、多くのタスクは瞬時に終わらないから
...
print("クラッシュ後の checkpoint:", runner.last_checkpoint)
...
### 3.2 なぜ checkpoint はタスクの最後だけに書いてはいけないのか？
...
### 5.2 checkpoint だけ保存して、ログを保存しない
...
この流れがきちんと設計できれば、  
システムは「たまに動く demo」から、「失敗しても続けられる本番システム」へ進化します。
