...
### 1.2 Planner の役割：大きな仕事を小さな仕事に分ける
...
### 1.3 Executor の役割：今のステップをしっかりやり切る
...
### 2.2 Plan-and-Execute は、先に作業表を作るイメージに近い
...
![Plan-and-Execute モニタリングと再計画の図](/img/course/ch09-plan-execute-monitor-replan-map.png)
...
### 3.3 これは単なる `for step in plan` より、なぜ学ぶ価値があるのか？
...
### 6.2 各ステップの実行後に必ず context に書き戻す
...
### 7.1 勘違い1：plan があれば、必ずもっと賢くなる

計画は安定性を上げます。  
でも前提として、計画そのものの質が十分である必要があります。

### 7.2 勘違い2：すべてのタスクで、必ず planner → executor にするべき

そんなことはありません。  
短いタスクや、対話性が強いタスクでは、ReAct のほうが自然なことが多いです。

### 7.3 勘違い3：計画はステップ名だけ書けば十分
