...
> **まず プランナー で計画を分解し、そのあと executor が計画どおりに順番に実行する。**
...
### プランナー（プランナー）の役割：大きな仕事を小さな仕事に分ける

プランナーの一番大事な価値は、「もっと賢いこと」ではなく、
次のことです。
...
:::tip 図の読み方
図を見るときは、2つの役割に注目してください。プランナーは全体のルートを担当し、実行器は今のステップを担当します。監視役は、情報不足・ツール失敗・目標変更を見つけたら、無理に元の計画を進めるのではなく再計画を起こします。
:::
...
### 勘違い2：すべてのタスクで、必ず プランナー → executor にするべき
...
2. なぜ長いタスクは短いタスクより プランナー が必要になりやすいのでしょうか？
...
3. replan は目標変更を検出し、実行を一時停止し、完了済みと未完了のステップを比較し、plan を更新し、なぜ変えたのかの trace を残すべきです。
4. ReAct は observation によって次の行動が決まる短めの探索タスクに向きます。Plan-and-Execute は、既知のサブゴールと依存関係を持つ長めのタスクに向きます。
