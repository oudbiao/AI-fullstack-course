...
## 一、なぜ「先に手順を考える」と役立つのか？

### 1.1 なぜなら、多くの問題は一気にゴールへ飛べないから
...
### 1.2 CoT の核心は「たくさん書くこと」ではなく、「中間構造を見えるようにすること」
...
## 二、まず「直接答える」方法と「チェーン推論」の比較を見てみよう
...
problem = "商品の定価は80円です。2割引のあとさらに5円引きなら、最終価格はいくらですか？"


def bad_direct_answer(text):
    numbers = list(map(int, re.findall(r"\d+", text)))
    original, discount, minus = numbers
    # よくあるミス：「8折」を「8元引き」と誤解する
...
### 2.2 なぜ CoT は数学、論理、計画系タスクで特に有効なことが多いのか？
...
### 2.3 なぜ CoT は、すべてのタスクで使うべきとは限らないのか？
...
![チェーン推論と自己チェックの構造図](/img/course/ch09-cot-self-check-structure-map.png)
...
### 5.2 推論チェーンが長すぎると、自分で自分を迷わせることがある
...
### 5.3 外部公開のときは、そのまま見せるのが適切でない場合がある
...
```python
ticket = {
    "question": "注文は未発送です。定価80円の商品が2割引になり、さらに5円引きされた場合、返金額はいくらですか？",
...
def structured_reasoning(ticket):
    facts = [
        ...
        "商品原価は 80 元で、2 割引のあとさらに 5 元引き",
    ]
    calculation = ["80 * 0.8 = 64", "64 - 5 = 59"]
    decision = "返金額は 59 元で、元の支払い方法にそのまま返金する。"
...
### 7.1 誤解その1：CoT は、モデルを少しおしゃべりにすること
...
### 7.2 誤解その2：すべてのタスクでデフォルト CoT にすべき

違います。  
有効かどうかは、その問題が本当に複数ステップかどうかで決まります。

### 7.3 誤解その3：CoT があれば、答えは必ず正確になる
