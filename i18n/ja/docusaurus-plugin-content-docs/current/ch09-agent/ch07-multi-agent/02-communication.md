...
## 一、なぜ通信が多 Agent システムの核心問題になるのか？

### 1.1 多 Agent の最大のリスクは「仕事ができない」ことではなく、「互いに認識が揃わない」こと
...
### 2.1 直接メッセージパッシング（message passing）
...
### 2.2 共有状態（shared state / blackboard）
...
![Agent 間通信契約図](/img/course/ch09-multi-agent-communication-contract-map.png)
...
### 8.3 複数の Agent が同じメッセージを違う意味に解釈する
...
## 九、本番に近いシステムで通信を安定させるには？

### 9.1 統一されたメッセージプロトコルを使う
