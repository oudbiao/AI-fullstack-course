...
| プランナー（Planner） | 現在の状態から次の行動を決める | 計画は目標とツール呼び出しをつなぐ橋 |
| ツール schema（Tool schema） | 必須フィールド、型、未知の引数を検証する | 悪い schema は誤ったツール呼び出しにつながる |
...
| 標準 | `run_agent()` に `max_steps` パラメータを追加する | ループする planner が `stopped_max_steps` で止まる |
| 標準 | `EVAL_CASES` に `validation_error` を追加する | 間違った引数型が実行前に検出される |
| 発展 | `run_id` ごとに trace を別ファイル保存する | ログ全体を読まなくても 1 回の実行を復習できる |
