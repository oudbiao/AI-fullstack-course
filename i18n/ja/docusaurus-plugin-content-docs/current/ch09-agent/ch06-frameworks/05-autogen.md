...
## 一、AutoGen スタイルのいちばん核心的な直感は何か？

### 1.1 先にフローを図にするのではなく、まず役割を「会話させる」
...
## 二、なぜこの「対話型マルチ Agent」は自然に感じられるのか？
...
## 四、なぜ AutoGen は「コード実行」系の場面とよく結びつくのか？

### 4.1 この種の場面は、もともと多段階のフィードバックに向いているから
...
### 4.2 最小限の「書く -> 実行する -> フィードバックする」のイメージ
...
## 六、ただし AutoGen スタイルのリスクも理解しておこう

### 6.1 メッセージの往復回数が簡単に増えすぎる
...
## 七、CrewAI、LangGraph との違いはどこにあるのか？
