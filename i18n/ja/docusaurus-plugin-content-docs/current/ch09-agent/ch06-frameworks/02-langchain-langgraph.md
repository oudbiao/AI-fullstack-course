...
![LangGraph 状態機械と条件付きエッジの図](/img/course/ch09-langgraph-state-machine-map-ja.webp)
...
2. チェーン型の例に「文書が見つからなかったら クエリ を書き換えてもう一度検索する」ロジックを追加し、どれくらい複雑になるか確認してみましょう。
...
3. graph abstraction が stateful loop に向くのは、node と edge が遷移を明示するからです。どの state が入り、何を判断し、次にどこへ進み、いつ止まるかを表せます。
4. chain が向くのは、タスクが線形で短く、安定していて確認しやすい場合です。たとえば入力整形、retriever 呼び出し、短い回答生成のような流れです。
