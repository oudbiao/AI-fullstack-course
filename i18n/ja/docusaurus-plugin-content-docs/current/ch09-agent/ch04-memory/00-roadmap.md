...
## 9.4.1.1 まず memory loop を見る

![Agent memory system の階層図](/img/course/agent-memory-system-ja.png)

![Agent memory systems 章の学習順序図](/img/course/ch09-memory-chapter-flow-ja.png)

![Agent memory writing and retrieval の閉ループ図](/img/course/ch09-memory-write-retrieve-loop-ja.png)

重要なのは「全部保存」ではありません。何を保存し、いつ retrieve し、いつ update し、いつ forget するかです。

## 9.4.1.2 Memory write filter を動かす
