...
## 二、書き込み方針：どんな情報がロングタームメモリに入る価値があるのか？

### 2.1 すべてのメッセージを保存する必要はない
...
![メモリ工学のライフサイクル図](/img/course/ch09-memory-engineering-lifecycle-map-ja.png)
...
### 5.2 なぜ「デバッグ用マーカー」が消えるのは妥当なのか？

それは一時情報で、`ttl_steps=1` が設定されているからです。  
その後も残しておくと、検索結果を汚すだけになることが多いです。

### 5.3 なぜ「ユーザーの好み」と「返金ポリシー」が優先的に呼び出されるのか？
