...
```text
manual_rule: h_t depends on x_t and h_{t-1}
shape_rule: x is [batch, seq_len, input_size]
output_rule: out has one hidden vector per step
classification_rule: many-to-one can use the last step
limit: plain RNN struggles with long dependencies
```
