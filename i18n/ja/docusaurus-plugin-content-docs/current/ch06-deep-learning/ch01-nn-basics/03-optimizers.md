...
3. AdamW は weight decay を Adam の更新から分離して扱うため、正則化の挙動をより読みやすくできます。
4. `w.grad` の符号と大きさを見ると、重みがどちらにどれだけ動くかを説明できます。
5. グラフでは最終値だけでなく、振動、停滞、滑らかさも比較します。optimizer の違いは曲線に出ます。
