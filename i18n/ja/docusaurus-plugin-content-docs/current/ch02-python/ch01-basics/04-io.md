...
1. `input()` は文字列を返します。年齢は `int()`、距離は `float()` に変換してから計算します。
2. 自己紹介の出力には上下の区切り線を入れ、内容はユーザーが入力した値を反映させます。
3. レシートは列をそろえ、与えられたデータで小計 `119.80`、`46.50`、合計 `166.30` を表示します。
4. `km = 5` のとき、マイルは約 `3.11`、メートルは `5000.00` です。
