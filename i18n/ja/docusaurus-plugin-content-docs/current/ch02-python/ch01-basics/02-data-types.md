...
:::tip 命名の黄金ルール
変数名は、**見ただけで何を表すか分かる**ようにしましょう。少し長くなっても（`student_count`）、意味の分からない略語（`sc`）よりずっとよいです。
:::
...
1. 個人情報カードでは `str`、`int`、`bool` を使います。f-string では変数の値に加え、`age + 1` のような式も表示できます。
2. `37.5` 度 Celsius は `99.5` 度 Fahrenheit です。`celsius` を変えると `fahrenheit` も変わるよう、答えを固定せず式をコードに残します。
3. 正規化後のメールアドレスは `zhangsan@example.com` です。空白を除き小文字にした後の `@` のインデックスは `8`、ユーザー名は `zhangsan` です。
