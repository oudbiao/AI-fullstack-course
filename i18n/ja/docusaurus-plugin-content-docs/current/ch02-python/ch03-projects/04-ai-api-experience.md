...
response = client.chat.completions.create(
    model="gpt-3.5-turbo",
    messages=[
        {"role": "user", "content": "こんにちは！ Python 言語を一文で紹介してください。"}
    ]
)
...
```python
messages = [
    {"role": "system", "content": "あなたは Python プログラミングのアシスタントです。"},  # システムプロンプト（AI の役割を設定する）
    {"role": "user", "content": "リスト内包表記とは何ですか？"},           # ユーザーのメッセージ
    {"role": "assistant", "content": "リスト内包表記は..."},         # 以前の AI の返答
    {"role": "user", "content": "例を出してもらえますか？"},               # ユーザーの追加質問
]
```

| 役割 | 意味 |
|------|------|
| `system` | AI の振る舞いと役割を設定する（任意） |
| `user` | ユーザーが送るメッセージ |
| `assistant` | 以前の AI の返答（多輪対話に使う） |
    ...
    while True:
        ...
        try:
            ...
            response = client.chat.completions.create(
                model="gpt-3.5-turbo",
                messages=messages,
                temperature=0.7,    # 創造性を調整する（0=保守的, 2=かなり自由）
                max_tokens=1000,    # 最大返信長
            )
            ...
            print(f"\n  [Token 使用量: 入力={usage.prompt_tokens}, "
                  f"出力={usage.completion_tokens}, "
                  f"合計={usage.total_tokens}]")

        except Exception as e:
            print(f"\n❌ 呼び出し失敗: {e}")
            messages.pop()  # 失敗したユーザーのメッセージを削除する
...
def review_code(code: str) -> str:
    ...
    response = client.chat.completions.create(
        ...
        messages=[
            {
                "role": "system",
                "content": "あなたは経験豊富な Python コードレビューの専門家です。"
                          "ユーザーのコードをレビューし、問題点を指摘して改善提案をしてください。"
                          "中国語ではなく日本語で返答し、わかりやすい形式で出力してください。"
            },
            {
                "role": "user",
...
def summarize(text: str, max_sentences: int = 3) -> str:
    ...
    response = client.chat.completions.create(
        ...
        messages=[
            {
                "role": "system",
                "content": f"次のテキストの核心内容を、{max_sentences} 文以内で要約してください。日本語で返答してください。"
            },
...
def translate(text: str, target_lang: str = "英語") -> str:
    ...
    response = client.chat.completions.create(
        ...
        messages=[
            {
                "role": "system",
                "content": f"あなたはプロの翻訳者です。ユーザー入力のテキストを{target_lang}に翻訳してください。"
...
多くの国産 API は OpenAI 互換のインターフェース形式に対応しており、`base_url` を変更するだけで使えます。
...
| パラメータ | 意味 | 推奨値 |
|------|------|--------|
| `model` | どのモデルを使うか | `gpt-3.5-turbo`（安い）, `gpt-4`（より強力） |
| `temperature` | 創造性/ランダム性 | 0.0-0.3（事実重視）, 0.7-1.0（創造的） |
| `max_tokens` | 最大出力長 | 必要に応じて設定する |
| `messages` | 対話履歴 | 完全な会話コンテキストを含める |
| `stream` | ストリーミング出力するか | `True` で টাইピング風の表示を実現 |
...
# Token 使用量を確認する
usage = response.usage
print(f"入力 Token: {usage.prompt_tokens}")
print(f"出力 Token: {usage.completion_tokens}")
print(f"合計 Token: {usage.total_tokens}")
```

:::tip コストを抑える
- `gpt-4` ではなく `gpt-3.5-turbo` を使う（10〜20倍安い）
- `max_tokens` を制御して、不要に長い返答を避ける
