...
### 3.3 さらに「スライディングウィンドウで動作を見る」最小例を追加しよう
...
### 4.3 単一フレームの精度だけを見て、時系列の安定性を見ない
