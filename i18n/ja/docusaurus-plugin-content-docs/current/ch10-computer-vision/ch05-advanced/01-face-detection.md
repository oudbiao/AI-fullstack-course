...
![顔検出、アライメント、Embedding と閾値リスク図](/img/course/ch10-face-recognition-threshold-pipeline-map.png)
...
### 3.1 検出だけ見て、アライメントを見ない

アライメントは、その後の認識の安定性に直接影響することがあります。

### 3.2 類似度だけ見て、閾値リスクを見ない

閾値が広すぎると誤認識しやすく、  
厳しすぎると見逃しやすくなります。

### 3.3 プライバシーと法令順守を無視する

顔タスクには、もともと高い法令順守要件が伴いやすいです。

### 3.4 成功した認識だけを示し、誤認識や拒否を示さない
...
### 4.2 これをプロジェクトにするなら、何を最初に見せるべきか
