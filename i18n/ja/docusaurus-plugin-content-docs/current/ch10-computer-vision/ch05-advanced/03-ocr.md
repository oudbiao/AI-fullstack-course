...
![OCR の検出枠、認識テキスト、読む順序の実行結果図](/img/course/ch10-ocr-region-text-reading-order-result-map-ja.webp)

1行目は検出結果です。文字領域がどこにあるかを示します。2行目は認識結果です。検出された各領域に何と書かれているかを示します。
...
### なぜ OCR のエラーは「文字そのものの認識」以外でも起きるのか？
...
元のリストは順番がばらばらですが、縦位置で並べ替えると、この単純な文書では上から下への自然な読み順に戻ります。
...
![OCR の検出、認識、読み順復元の図](/img/course/ch10-ocr-layout-reading-order-map-ja.webp)
...
### OCR をプロジェクトにするなら、最初にどんな題材を選ぶべき？
...
### OCR をプロジェクトとして見せるなら、何を最初に見せるとよい？
...
この小さな表は、次にどこを見るべきかを示します。文字ボックスの見落とし、文字の誤認識、文書順序の誤りは、それぞれ違う修正が必要です。
