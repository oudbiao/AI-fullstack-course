...
![画像配列の Shape とチャンネルの意味図](/img/course/ch10-image-array-shape-channel-map-ja.webp)
...
### 画像は「オブジェクト」であって「配列」ではないと思ってしまう
...
3. 正規化は各チャンネル値を `255.0` で割ることです。色の相対関係は変わりませんが、数値範囲が `0-1` になり、モデルや後続処理で扱いやすくなります。
