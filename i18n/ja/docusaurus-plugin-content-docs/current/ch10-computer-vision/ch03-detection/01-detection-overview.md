...
これは、2つのボックスの重なりが合計領域の半分未満であることを意味します。実際の検出器でこれを正解とみなすかどうかは、`0.5` や `0.75` など、設定した IoU 閾値によって変わります。
...
![物体検出出力、IoU とエラータイプの図](/img/course/ch10-detection-output-iou-error-map-ja.webp)
...
3. クラスが正しくても位置が大きく外れている予測はよい予測とは言えません。IoU しきい値を満たさず、後続の処理も誤らせる可能性があります。
4. 複数物体のシーンでは、スケール差、遮蔽、box の重なり、重複予測、クラス混同が加わるため、単一物体よりかなり難しくなります。
