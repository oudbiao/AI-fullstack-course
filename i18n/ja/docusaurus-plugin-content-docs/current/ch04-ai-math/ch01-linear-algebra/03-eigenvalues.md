...
![固有ベクトルの特別な方向のイメージ図](/img/course/eigenvalue-special-directions.png)
...
![固有方向と PCA の地図](/img/course/ch04-eigen-pca-direction-map.png)
...
data[:, 1] = data[:, 0] * 0.8 + np.random.randn(100) * 0.3  # 相関を作る
...
n = 200
x = np.random.randn(n)
y = 0.6 * x + np.random.randn(n) * 0.3  # y と x に関連がある
