...
keywords: [AI 数学ワークショップ, ベクトル類似度, 確率シミュレーション, エントロピー, 勾配降下, 数学証拠パック]
---

# 4.4 実践：第 4 章フル数学ワークショップ
...
## 4.4.2 図解チェックポイント：全体ルート

コードを書く前に、次の図をワークショップの地図として見てください。

![第 4 章数学実践ワークショップのルート図](/img/course/ch04-hands-on-math-workshop-route-ja.png)

全体の流れは、小さな数値から始め、コードで動かし、最後に見える証拠を残すことです。

![ベクトル類似度の証拠フロー図](/img/course/ch04-hands-on-vector-similarity-flow-ja.png)

ベクトルのステップでは、クエリベクトルに最も近い向きを持つトピックを探します。

![確率シミュレーションと不確実性の流れ図](/img/course/ch04-hands-on-probability-simulation-flow-ja.png)

確率のステップでは、モデルスコアが魔法の正解ではなく、サンプル全体の不確実性を要約する方法だと分かります。

![勾配降下の実行ループ図](/img/course/ch04-hands-on-gradient-descent-loop-ja.png)

勾配降下のステップでは、loss を計算し、傾きを計算し、パラメータを更新し、繰り返すという訓練のリズムを見ます。

![第 4 章数学証拠パック図](/img/course/ch04-hands-on-evidence-pack-ja.png)
