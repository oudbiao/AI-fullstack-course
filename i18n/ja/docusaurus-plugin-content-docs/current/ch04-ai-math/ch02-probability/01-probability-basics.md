...
![確率分布とベイズ更新の図](/img/course/probability-distribution-map-ja.png)
...
![確率とベイズ更新のフローチャート](/img/course/ch04-probability-bayes-update-flow-ja.png)
...
np.random.seed(42)
n_flips = 10000
results = np.random.choice(['表', '裏'], size=n_flips)
...
### 2.1.1 なぜ条件付き確率は AI の考え方にそっくりなの？
...
# 天気：晴れ(0.7) / 雨(0.3)
weather = np.random.choice(['晴れ', '雨'], n, p=[0.7, 0.3])

# 傘を持つ確率は天気によって変わる
umbrella = np.where(
    weather == '雨',
    np.random.choice(['持つ', '持たない'], n, p=[0.8, 0.2]),  # 雨の日は 80% が傘を持つ
    np.random.choice(['持つ', '持たない'], n, p=[0.1, 0.9])   # 晴れの日は 10% が傘を持つ
)

# 同時確率表
import pandas as pd
df = pd.DataFrame({'天気': weather, '傘': umbrella})
joint = pd.crosstab(df['天気'], df['傘'], normalize=True)
print("同時確率表：")
print(joint.round(3))
...
test_positive = np.where(
    has_disease,
    np.random.random(n_sim) < p_positive_if_disease,  # 病気
    np.random.random(n_sim) < p_positive_if_healthy    # 病気でない
)
...
- [確率分布：データの背後にある規則](./02-distributions.md)
...
| 概念 | 直感 | 公式/コード |
|------|------|----------|
| 確率 | 不確実性の測り方（0〜1） | `np.random.random() < p` |
| 条件付き確率 | B が起きたときの A の確率 | P(A\|B) = P(AかつB) / P(B) |
| 同時確率 | A と B が同時に起こる確率 | `pd.crosstab(normalize=True)` |
