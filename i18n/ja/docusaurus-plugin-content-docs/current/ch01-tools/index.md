...
手順に沿って進めたい場合は、[1.4 ハンズオンワークショップ：再現可能な AI 学習ラボを作る](ch04-workshop/01-hands-on-tools-workshop.md) に進んでください。段階プロジェクトを、ローカルリポジトリ、実行できる Python チェックスクリプト、Git commit、トラブルシュート記録、レポートファイルに分解して進めます。
...
| 推奨クリア | この段階の少なくとも 1 つの実行可能な小プロジェクトを完成させる。できれば 1.4 のハンズオンを完了し、README に実行方法、入力と出力の例、起きた問題を記録できる。 |
