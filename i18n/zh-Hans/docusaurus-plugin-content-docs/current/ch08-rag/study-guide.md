...
用这些任务把 RAG 落到实处。RAG 项目不是“能回答”就完成，而是要能检查检索证据、给出来源引用、评估失败，并能稳定复跑。
...
| 完成实操工作坊 | `rag_app_workshop.py` 输出和一个新增评估用例 | 能复现预期输出，解释每个阶段，并安全扩展 |
...
在扩展项目之前，先完成 [8.5.6 实操：第 8 章 RAG 应用完整工作坊](./ch05-projects/05-stage-hands-on-workshop.md)。它是从概念阅读过渡到作品集知识库助手的推荐实践桥梁。
...
| `rag_app_workshop_output.txt` | [8.5.6 实操：第 8 章 RAG 应用完整工作坊](./ch05-projects/05-stage-hands-on-workshop.md) 的运行输出，以及你做的一处小改动说明 |
| `README.md` | 运行命令、示例输入输出、评估结果和改进计划 |

这些文件会让你的 RAG 项目从“能回答”变成“能解释、能评估、能复盘”。

## 阶段通关问题

学完本阶段后，你应该能独立完成一个 RAG 应用，能解释数据从文档到答案的完整路径，并能定位效果差可能发生在哪一层。

进入第 9 章前，确认自己能回答这些问题：

- RAG 解决了大模型的什么限制？
- chunk 大小会影响什么？
- embedding 检索和关键词检索有什么区别？
- 为什么答案必须有引用？
- 如何判断一次 RAG 失败是检索问题还是生成问题？
