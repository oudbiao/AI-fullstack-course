...
![RAG 从问题到证据的工作闭环](/img/course/ch08-rag-basics-workflow-map-v2.png)

这张图先帮你把主线记住：提问、检索、带证据作答。
...
如果你想有人带着做，先完成 [8.5.6 实操：第 8 章 RAG 应用完整工作坊](./ch05-projects/05-stage-hands-on-workshop.md)。它用一个可运行脚本串起 chunk、metadata、检索、权限、引用、无答案处理、trace 和小型评估集。
...
![RAG 应用系统主干图](/img/course/ch08-rag-system-backbone.png)
...
![RAGOps 持续改进闭环图](/img/course/ch08-ragops-improvement-loop.png)
...
8.1 节学习 RAG。你会理解文档解析、切分、Embedding、向量数据库、检索策略、RAG 优化和评估。

8.2 节学习 LLM 本地部署与统一接口。你会理解本地模型、推理服务和统一 API 的意义。

8.3 节学习大模型应用开发，包括 LLM API、LangChain 基础、Function Calling、对话系统、文档解析、AI 辅助编码和模板文档生成。

8.4 节学习工程化实践，包括异步编程、API 设计、日志监控和 Docker 部署。

8.5 节进入综合项目，把知识库、模型调用、应用接口和工程化组合起来。

如果你希望先通过动手建立整体感，可以先做8.5 节的完整实操工作坊。它是本阶段的实践主线：先复现输出，再回到 RAG、API、Function Calling、日志和部署章节，逐个替换简单组件。
...
这个实验的更完整跟做版在 [8.5.6 实操：第 8 章 RAG 应用完整工作坊](./ch05-projects/05-stage-hands-on-workshop.md)。建议把它当作做作品集 RAG 助手前的最小可运行项目。
...
如果你想看更细的学习节奏，可以阅读 [8.0 学习指南：大模型应用与工程怎么学最不容易学乱](./study-guide.md)。
