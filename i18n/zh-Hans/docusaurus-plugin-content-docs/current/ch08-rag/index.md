...
![RAG 学习闯关地图](/img/course/ch08-learning-quest-map.png)
...
如果你想有人带着做，先完成 [5.6 实操：第 8 章 RAG 应用完整工作坊](./ch05-projects/05-stage-hands-on-workshop.md)。它用一个可运行脚本串起 chunk、metadata、检索、权限、引用、无答案处理、trace 和小型评估集。
...
如果你希望先通过动手建立整体感，可以先做第五章的完整实操工作坊。它是本阶段的实践主线：先复现输出，再回到 RAG、API、Function Calling、日志和部署章节，逐个替换简单组件。
...
这个实验的更完整跟做版在 [5.6 实操：第 8 章 RAG 应用完整工作坊](./ch05-projects/05-stage-hands-on-workshop.md)。建议把它当作做作品集 RAG 助手前的最小可运行项目。
...
| 玩法 | 本阶段任务 |
|---|---|
| 剧情任务 | 让助手基于资料回答：导入文档、检索片段、生成答案，并检查引用是否支持结论。 |
| Boss 战 | **引用幻觉龙** |
| 可解锁徽章 | RAG 引用警察、检索考古学家 |
| 新手轻松版 | 只完成一个最小输入到输出闭环，先留下运行截图或命令输出 |
| 作品集证据 | eval questions、retrieval logs、citation_check |
