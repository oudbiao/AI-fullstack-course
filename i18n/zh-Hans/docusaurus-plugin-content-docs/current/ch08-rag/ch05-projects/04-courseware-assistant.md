...
```text
{'title': '折扣应用题', 'concepts': ['折扣 = 原价 × 折扣率'], 'examples': ['商品原价 100 元，打 8 折后价格是多少？'], 'exercises': ['一件衣服原价 80 元，打 7 折后是多少元？'], 'notes': ['外部资料补充：折扣应用题 的常见教学误区。']}
```
...
导出 Word 之前，先检查每个必需栏目是否有内容。这样可以避免模板层生成一个“格式很好看，但正文是空的”文档。

```python
courseware = build_courseware("折扣应用题")
