...
|---|---|---|---|
| 正确资料完全没出现 | 查询、top-k 原始命中、chunk 文本 | 调整切块、关键词检索、查询改写 | 直接换更大生成模型 |
| 正确资料出现但排得靠后 | 每个 chunk 的 score 和排序 | 加 rerank、调混合检索权重 | 盲目把 top-k 拉很大 |
| 正确资料在 上下文 里但答案漏条件 | 最终 上下文、prompt、答案引用 | 调整 上下文组装、要求逐条引用 | 只改 embedding 模型 |
| 答案引用了错误来源 | answer、source_refs、证据片段 | 做 citation check、限制引用格式 | 只看最终答案是否流畅 |
| 延迟和成本突然升高 | top-k、rerank 数量、上下文 长度 | 限制候选数量、缓存、分层检索 | 同时增加 top-k 和模型大小 |
...
| 实验 | 改动 | 改善 | 代价 | 结论 |
|---|---|---|---|---|
| 基线 | 关键词检索，top-k=3 | 精确术语表现稳定 | 同义问法较弱 | 作为对照组保留 |
| exp-1 | 加 查询改写 | 同义问法命中提升 | 少量错误改写 | 保留，但要记录改写日志 |
