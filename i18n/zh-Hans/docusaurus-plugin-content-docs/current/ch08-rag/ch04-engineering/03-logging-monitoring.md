...
| `observability` | 能否从外部理解系统行为 | 日志、指标、追踪、仪表盘和告警的组合 |

新人先记住这个区别就够了：日志是单个事件，指标是聚合数字，追踪把一条请求的多个步骤串起来。
...
![日志、指标与 追踪 可观测性图](/img/course/ch08-observability-logs-metrics-trace-map.webp)
...
> **日志、指标、追踪 共同组成了系统的可观测性，它们决定你能不能真正维护一个上线后的 LLM 服务。**
