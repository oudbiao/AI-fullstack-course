...
## 如果你的目标是“知识库驱动的 SOP 文档助手”，哪些步骤最值得并发？

这类项目里，最容易并发起来的通常不是“最终生成 SOP 草稿”这一步，
而是生成前的几个外部等待动作。
...
3. LLM 应用经常等待模型 API、retriever、数据库、工具、存储和监控。async 能更好利用等待时间。
4. 它不会让单次远程调用变快，而是让这个调用等待时，系统还能推进其他有用工作。
