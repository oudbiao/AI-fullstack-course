...
3.1 节用纯 Python 处理数据，让你体会为什么需要 NumPy 和 Pandas。

3.2 节学习 NumPy。你会理解数组、向量化、广播、矩阵运算和随机统计，这些概念会在机器学习和深度学习里反复出现。

3.3 节学习 Pandas。你会处理表格数据、读写文件、筛选过滤、清洗缺失值、分组聚合、合并数据和处理时间序列。

3.4 节学习可视化。你会用图表表达分布、趋势、关系和异常，而不是只把结果打印成表格。

3.5 节数据库是选修，但如果你想做真实应用或企业数据项目，建议至少理解关系型数据库和 SQL 基础。

3.6 节会把前面分散的技能变成可展示的证据。如果你想先跟着一页完整跑通，推荐做 [3.6.3 跟做工作坊：搭建可复现的数据分析流水线](./ch06-projects/03-hands-on-data-workshop.md)：它会生成脏 CSV、完成清洗、写入 SQLite、生成图表并输出报告。
...
| 可复现实操证据 | 运行 6.3 流水线并保留生成文件 | 包含原始数据、清洗数据、清洗记录、SQLite 数据库、SVG 图表和 HTML 报告 |
