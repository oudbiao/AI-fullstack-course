...
miles = km * 0.621371
meters = km * 1000
print(f"{km:.2f} 公里 = {miles:.2f} 英里")
print(f"{km:.2f} 公里 = {meters:.2f} 米")
```

<details>
<summary>参考实现与讲解</summary>

1. `input()` 返回字符串。年龄要用 `int()` 转换，距离要用 `float()` 转换后才能做数学运算。
2. 自我介绍输出应该有上下分隔线，并且内容来自用户实际输入。
3. 购物小票应对齐列，并对给定数据算出小计 `119.80`、`46.50`，总计 `166.30`。
4. 当 `km = 5` 时，英里约为 `3.11`，米数是 `5000.00`。
