...
| 术语 | 这里是什么意思 | 为什么会出现在本章 |
|---|---|---|
| `RAG` | Retrieval-Augmented Generation，检索增强生成：先检索资料，再生成回答 | 检索需要向量相似度。 |
| `PCA` | Principal Component Analysis，主成分分析 | 它用主方向压缩或可视化数据。 |
| `X @ W + b` | 矩阵乘法再加偏置 | 很多神经网络层都可以写成这个基本形状。 |
| 点积 | 对应位置相乘再相加 | 它是相似度和矩阵乘法背后的核心运算。 |
| 余弦相似度 | 比较两个向量的夹角 | 当不希望向量长度主导相似度时，经常使用它。 |

如果这些词现在还不熟，不要为了背词典而停下来。把这张表当成随身小抄，后面再次遇到同一个词时再回来查。
...
样本矩阵形状: (3, 3)
单个学生综合分: 89.8
所有学生综合分: [89.8 75.6 91.8]
```

如果输出不同，先检查权重是否仍然加起来等于 `1.0`，样本矩阵形状是否仍然是 `(3, 3)`，以及你是否用了 `@` 做矩阵/向量乘法，而不是用 `*` 做逐元素相乘。
