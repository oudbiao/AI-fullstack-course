...
学完 `4.1`、`4.2`、`4.3` 后，再运行 [4.4 实操：第 4 章完整数学工作坊](./hands-on-math-workshop.md)。它会把向量相似度、概率模拟、熵/损失、梯度下降、SVG 图和可复盘证据文件夹串在一起。
...
4.1 节学习线性代数。你需要理解向量、矩阵、矩阵乘法、线性变换和特征值这些概念如何出现在数据矩阵、Embedding、神经网络参数和注意力计算中。

4.2 节学习概率与统计。你需要理解概率、分布、期望、方差、统计推断和信息熵，它们会出现在分类模型、损失函数、评估指标和生成模型里。

4.3 节学习微积分与优化。你需要理解导数、偏导、梯度、链式法则和梯度下降，因为它们解释了模型如何通过损失函数一点点更新参数。
...
学完理论小节后，再运行 [4.4 实操：第 4 章完整数学工作坊](./hands-on-math-workshop.md)，并把生成的 `ch04_math_workshop_evidence` 文件夹保存为第一个项目产物。

如果你想看更细的学习节奏，可以阅读 [4.0 学习指南：AI 数学基础怎么学最不容易放弃](./study-guide.md)。
