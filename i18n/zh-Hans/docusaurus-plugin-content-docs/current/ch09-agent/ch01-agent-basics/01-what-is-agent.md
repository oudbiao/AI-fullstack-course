...
1. 新工具要有清晰的名称、输入格式、返回格式和失败方式。比如 `check_course_schedule` 不应返回一大段散文，而应返回较小的结构化结果。
2. 两步任务需要在步骤之间保存状态：文档检索结果要被存起来，再传给计算步骤，同时要有最大步数保护，避免循环失控。
3. Agent 不应该把工具错误藏起来，也不应该把错误当作事实继续执行。更安全的做法是先分类错误，只在安全时重试，必要时请求补充信息，或者返回受控的失败说明。
