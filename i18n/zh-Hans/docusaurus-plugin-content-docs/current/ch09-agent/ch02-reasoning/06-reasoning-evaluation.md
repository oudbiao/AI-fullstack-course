...
![Agent 推理失败类型诊断图](/img/course/ch09-reasoning-eval-failure-taxonomy-map.webp)
...
1. 增加一个 case 可能改变 accuracy、tool-use rate、loop count 或失败分布；关键是看哪个指标变化，以及为什么变化。
2. 只看最终正确率会漏掉坏 trace：重复工具调用、不安全动作、缺少引用、成本过高，或者只是碰巧答对。
3. 优先检查 routing 和停止逻辑，包括工具描述、observation 解析、循环终止条件和 retry policy。
4. 有价值的指标包括任务成功率、step count、无效工具调用率、恢复率、延迟与成本、groundedness，以及用户可见错误率。
