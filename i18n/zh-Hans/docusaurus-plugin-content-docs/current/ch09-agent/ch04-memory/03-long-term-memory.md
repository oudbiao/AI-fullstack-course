...
![长期记忆写入、更新与置信度图](/img/course/ch09-long-term-memory-write-update-policy-map.webp)
...
3. 偏好频繁变化时，保留版本并做 confidence decay 通常比盲目 overwrite 更安全，因为它保留历史，也会降低旧记忆影响。
4. 长期记忆保存稳定偏好和事实；短期记忆保存当前目标、约束、最近修正和检索证据。当前回答要把两者合并使用。
