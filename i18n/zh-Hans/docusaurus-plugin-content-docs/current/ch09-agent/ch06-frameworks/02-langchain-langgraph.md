...
![LangGraph 状态机与条件边图](/img/course/ch09-langgraph-state-machine-map.webp)
...
3. graph 抽象适合有状态循环，是因为节点和边把转移显式化：什么状态进入、做了什么判断、下一步去哪、什么时候停止。
4. 当任务线性、短、稳定、容易检查时，chain 更合适，比如格式化输入、调用一次 retriever、生成一段短回答。
