...
| 层级 | 现在学什么 | 怎么使用 |
|---|---|---|
| 必修核心 | 单 Agent 闭环、工具 schema、白名单、最大步数、状态边界、记忆边界、trace 日志、安全拦截、评估用例 | 这是构建可复盘 Agent 的最小能力，而不只是做一个演示 |
| 可选扩展 | MCP、框架比较、多 Agent 协作、部署运维、成本优化 | 单 Agent 闭环稳定后，项目需要集成或规模化时再回来 |
| 深度挑战 | 用同一个任务分别做成工作流、RAG flow、Function Calling 和 Agent trace，再说明哪个是最简单安全设计 | 避免为了赶潮流而使用 Agent |
...
| 项目可用 | 能定义工具 schema、拦截白名单外工具、设置最大步数，并保存失败追踪记录。 |
