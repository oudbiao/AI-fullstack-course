...
如果你想在选择大项目之前先跑一个具体 baseline，请先完成 [实操：构建一个可追踪的单 Agent 助手](./ch10-projects/04-stage-hands-on-workshop.md)。它会带你跑通单 Agent 循环、工具 schema 校验、权限拦截、JSONL trace 日志和评估用例。
