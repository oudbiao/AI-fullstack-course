...
9.1 节学习 Agent 基础概念，理解 Agent 和聊天机器人的区别、发展历史、能力层级和系统架构。

9.2 节学习推理与规划，包括 Chain-of-Thought、ReAct、Plan-and-Execute 和推理评估。

9.3 节学习工具使用与 Function Calling。你会理解工具描述、参数设计、调用策略、安全边界和代码执行型 Agent。

9.4 节学习记忆系统，包括短期记忆、长期记忆、情景记忆、程序性记忆和记忆工程。

9.5 节学习 MCP，理解模型和外部工具生态如何通过协议连接。

9.6 和 9.7 节学习 Agent 框架与多 Agent 系统，包括 LangGraph、LlamaIndex、CrewAI、AutoGen 等。

9.8 到 9.10 节学习评估、安全、部署和综合项目。
...
如果你想在选择大项目之前先跑一个具体 baseline，请先完成 [9.10.5 实操：构建一个可追踪的单 Agent 助手](./ch10-projects/04-stage-hands-on-workshop.md)。它会带你跑通单 Agent 循环、工具 schema 校验、权限拦截、JSONL trace 日志和评估用例。

如果你想看更细的学习节奏，可以阅读 [9.0 学习指南：Agent 系统怎么学最不容易学乱](./study-guide.md)。
