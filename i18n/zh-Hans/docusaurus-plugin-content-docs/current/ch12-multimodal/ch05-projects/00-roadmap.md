...
第 12 章 AIGC 与多模态是方向拓展与毕业项目阶段，而综合项目是它的出口。它要证明你不只是知道几个生成模型，也能把输入、生成、编辑、审核、导出和记录组织成闭环。
...
推荐的第一个可运行例子是 [12.5.3 实操：构建一个可复现的多模态创意包](./02-hands-on-multimodal-workshop.md)。如果你想先建立一个关于 brief 输入、Prompt 记录、资产版本、分镜导出、安全审核和失败分析的具体 baseline，请在更大的创意平台页之前先运行它。
...
|---|---|
| 案件名 | 多模态素材失控案 |
| 案发现场 | 生成或理解结果看起来不错，但素材来源、审核标准或导出限制不清。 |
| 侦查步骤 | 记录输入素材、生成参数、人工审核和失败/边界样例。 |
| 结案证据 | 素材来源表、审核记录、失败样本和导出说明。 |

项目练习不要只保留成功截图。至少挑一个真实失败样本，按“现象、线索、嫌疑原因、侦查步骤、修复动作、回归检查”写进 `reports/failure_cases.md`，这样项目会更像真实工程作品。

## 项目交付物标准

每个 AIGC 与多模态综合项目都建议按同一套作品集标准交付，而不是只展示一张生成结果。最小交付物应该包括：一份 README、一条可复现运行命令、一组示例输入输出、一张关键流程图、一次失败样本分析，以及下一步改进计划。

| 交付物 | 最低要求 | 进阶要求 |
|---|---|---|
| README | 写清项目目标、运行方式、依赖、素材来源和示例 | 增加架构图、设计取舍、审核流程和复盘 |
| 示例输入输出 | 至少保留 1 个完整多模态案例 | 保留成功、失败、边界和人工编辑案例 |
| 评估记录 | 写清用什么标准判断生成或理解效果 | 加入质量评分、事实检查、引用检查和用户反馈 |
| 安全审核记录 | 记录版权、肖像、敏感内容和使用边界 | 增加人工确认、导出限制和风险说明 |
| 展示材料 | 截图或短 GIF 证明能运行 | 做成可讲解的多模态作品集页面 |

做多模态项目时最重要的不是生成结果多好看，而是能讲清楚：素材从哪里来，Prompt 和版本如何管理，输出如何审核，最终交付如何避免版权、肖像、事实和安全风险。
