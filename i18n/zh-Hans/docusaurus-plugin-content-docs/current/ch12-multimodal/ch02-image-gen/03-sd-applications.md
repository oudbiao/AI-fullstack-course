...
这样理解后，为什么它会自然长成产品，而不只是模型演示，就会清楚很多。
...
![Stable Diffusion 应用模式输出对比图](/img/course/ch12-sd-mode-output-comparison.webp)

:::tip 按必需输入做路由
只有 prompt 的请求可以从空白画布开始；带 image 的请求要保留已有结构；带 mask 的请求才知道只改哪一个局部区域。
:::
