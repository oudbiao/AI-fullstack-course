...
3. 教育虚拟讲师至少需要脚本或对话生成、TTS、形象渲染、唇形同步、课程状态、检索或课程内容 grounding、安全评审，以及学习进度分析。
4. 稳定性和成本重要，是因为数字人常常用于长时间、重复使用的场景。略微不那么逼真但可靠、可负担的系统，比昂贵且不稳定的系统更容易落地。
