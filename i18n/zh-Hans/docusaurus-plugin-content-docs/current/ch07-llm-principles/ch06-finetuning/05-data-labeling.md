...
![数据标注质检与飞轮回流图](/img/course/ch07-data-labeling-flywheel-review-map.webp)
...
1. 好标签应彼此可区分、能指导动作，并用边界案例说明。正例和反例要让判断规则可见，而不只是重复标签名。
2. 一致率显示原始一致程度，kappa 会扣除随机一致的影响。如果一致性低，常见原因是说明不清、标签重叠或样本本身模糊。
3. 优先回流高频失败、高风险失败，以及代表稳定模式的失败样本。偶发的小错误通常不如反复影响真实用户的失败模式有价值。
4. 应先检查规范或标签体系是否含糊。投票可以解决单个样本，但持续争议说明任务定义本身需要修复。
