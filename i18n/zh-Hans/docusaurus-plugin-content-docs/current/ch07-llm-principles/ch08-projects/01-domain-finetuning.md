...
![垂直领域微调项目闭环图](/img/course/ch07-domain-finetune-closed-loop.webp)
...
| Prompt | 推理时发给模型的指令、上下文和约束 | 它是第一个 基线，因为成本比训练低 |
...
| 基线 | 在复杂方法之前先做的最简单对比系统 | 防止没有证据就说“效果提升了” |
...
![垂直领域微调 基线 覆盖率运行结果图](/img/course/ch07-domain-finetune-baseline-coverage-result-map.webp)
...
![垂直领域微调项目评估看板图](/img/course/ch07-domain-finetune-evaluation-board-map.webp)
...
| 输出必须被代码稳定解析 | 先用 Prompt + 结构约束，再看是否需要微调 | 结构约束 约束能暴露问题到底在表达还是行为 |
| 基线 已经通过评估 | 保留简单方案 | 简单可靠的系统通常更容易维护 |
...
> **垂直领域微调项目真正有价值的地方，不是“我微调了一个模型”，而是你能否把任务边界、SFT 数据、基线、评估规则和 before/after 对比讲成一条清楚闭环。**
...
2. 想一想：如果 Retrieval 基线 已经很好，什么时候微调还值得做？
...
1. 新样本要覆盖短句、长句、模糊表达和噪声表达，让每个 intent 都有足够变化。均衡不是只追求数量相等，而是覆盖场景。
2. 如果需要稳定语气、领域表达、重复分类模式、较低 prompt 复杂度，或更紧凑的运行成本，微调仍然值得考虑。
3. 政策点覆盖率衡量的是必需内容是否出现；“像不像人工”太主观，可能奖励流畅但不完整的回答。
4. 最值得展示能修复明确失败的例子：intent 判断错误、政策点遗漏、拒答不当、格式不稳定。每个 before/after 都要附评估理由。
