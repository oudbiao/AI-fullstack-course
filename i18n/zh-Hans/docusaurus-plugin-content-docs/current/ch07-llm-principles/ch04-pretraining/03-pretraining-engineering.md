...
![预训练工程生产线图](/img/course/ch07-pretraining-engineering-production-line.webp)
...
3. Shard 大小不均会产生 straggler。有些 worker 先结束后等待，吞吐会抖动；如果调度控制不好，有效数据配比也可能漂移。
4. 预训练需要协调数据管道、分布式计算、checkpoint、监控、失败恢复、质量控制和成本管理。模型代码只是这个系统里的一个组件。
