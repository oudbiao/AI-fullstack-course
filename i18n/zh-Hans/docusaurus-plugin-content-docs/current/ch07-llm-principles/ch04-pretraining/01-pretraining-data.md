...
:::tip 读图提示
训练语料和评测集必须严格隔离。如果基准测试题目、答案或近似变体泄漏到训练中，模型可能只是见过类似答案模式，所以分数虚高。这不代表真实泛化能力。
:::
...
| 术语 | 含义 | 在预训练里为什么重要 |
|---|---|---|
| 数据污染 | 测试题、答案或近重复样本意外进入训练语料 | 模型可能记住基准测试模式，而不是学到可迁移能力 |
...
3. 面向代码的模型应提高代码、文档、issue 讨论、API reference、测试和调试 trace 的比例，同时保留足够自然语言来支持指令和解释。配比应服务目标产品行为，而不是套一个通用比例。
4. 如果评测样本进入训练，模型可能记住答案或学到表面模式。此时分数反映的是“见过”，而不是真正对未见任务的泛化能力。
