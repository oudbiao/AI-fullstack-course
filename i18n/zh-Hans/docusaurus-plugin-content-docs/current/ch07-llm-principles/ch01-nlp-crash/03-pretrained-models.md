...
3. 私有政策场景通常先从 RAG 开始，因为知识是私有的、会变化、还需要引用来源。微调更适合后续稳定行为模式、语气或格式。
4. 至少要做两个检查：用代表性数据做任务评估，以及复盘敏感失败样例。隐私、延迟、成本和偏差检查也是生产要求。
5. 更大的模型可能能力更广，但任务适配还取决于数据、指令、检索、评估和运行约束。参数规模只是决策中的一个因素。
