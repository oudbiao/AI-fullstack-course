---
title: "1.6 实操：Tokenizer 与 Embedding 实验室"
sidebar_position: 5
...
### 4.3 `attention_mask` 防止 padding 被当成意义

如果句子短于 `max_length`，代码会补 `[PAD]`。
mask 把 padding 标成 `0`，告诉模型这不是实际内容。

### 4.4 Embedding 是 ID 开始携带语义特征的地方
