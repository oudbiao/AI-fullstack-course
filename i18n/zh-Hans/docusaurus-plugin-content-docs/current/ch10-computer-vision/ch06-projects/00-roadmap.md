...
推荐的第一个可运行例子是 [10.6.4 实操：构建一个可复现的视觉迷你流水线](./03-hands-on-vision-workshop.md)。如果你想先建立一个关于数据生成、预处理、分类、检测框、分割 mask、指标和失败分析的具体 baseline，请在几个大项目页之前先运行它。
