...
这张漫画就是本节主线：PCA 不是“随便删掉几列”，而是把坐标系转到信息最集中的方向上，保留能解释最多变化的主方向，得到更紧凑、可能更少噪声的表示。t-SNE 和 UMAP 解决的是另一个问题：能不能把高维结构更好地看出来。

### 开始前先解码几个关键词

| 术语 | 新人友好的解释 | 在本节里的作用 |
|---|---|---|
| PCA | Principal Component Analysis，主成分分析；用线性方式从旧特征生成新轴 | 通常是建模预处理和压缩时最稳的第一选择 |
| PC1 / PC2 | 第 1、第 2 主成分 | 它们是新轴，不是原始字段 |
| `n_components` | 降维后想保留多少维 | 控制压缩力度 |
| `explained_variance_ratio_` | 每个主成分保留下来的方差比例 | 帮你判断 2、10、30、40 个主成分是否够用 |
| Scree Plot | 按主成分数量画出的方差解释图 | 帮你找到“继续增加主成分收益变小”的位置 |
| t-SNE | t-distributed Stochastic Neighbor Embedding，非线性可视化方法 | 适合看局部邻近关系，不适合直接当下游模型特征 |
| UMAP | Uniform Manifold Approximation and Projection，非线性降维方法 | 通常比 t-SNE 更快，有时也能用于特征提取 |
| `perplexity` | t-SNE 大概关注多少邻居 | 会改变可视化形状，建议试多个值 |
| `fit_transform()` | 先学习降维规则，再立即应用到数据上 | sklearn 预处理代码里很常见 |
...
| **特征提取** | 生成新特征 | 把原始特征变换成更少的新特征。PCA 是经典且适合建模的例子；t-SNE 主要用于可视化 |
...
这个结果很值得注意：把 64 个像素特征强行压到 2 维，非常适合可视化，但只保留了大约五分之一的方差。不要默认认为 2D PCA 图已经包含足够多的信息来训练高质量分类器。
...
```text
保留 90% 方差需要 31 个主成分（原始 64 个）
保留 95% 方差需要 40 个主成分（原始 64 个）
```
...
```text
PC=  2 | 准确率: 51.7% | 训练时间: 0.015s
PC=  5 | 准确率: 81.9% | 训练时间: 0.013s
PC= 10 | 准确率: 88.6% | 训练时间: 0.013s
PC= 20 | 准确率: 94.4% | 训练时间: 0.010s
PC= 30 | 准确率: 96.1% | 训练时间: 0.009s
PC= 64 | 准确率: 97.2% | 训练时间: 0.009s
```

训练时间会受电脑性能影响，不要背秒数。真正要观察的是取舍关系：主成分太少会丢信息，主成分变多通常能恢复准确率，但压缩效果会变弱。
...
`max_iter` 表示最多优化多少轮。在较新的 sklearn 版本中，推荐使用 `max_iter`；一些旧教程可能还写 `n_iter`，那是旧版本参数名。
...
安装 `umap-learn` 后的预期输出：

```text
UMAP result shape: (1797, 2)
```

如果你看到安装提示，代码本身也没有错，只是说明当前 Python 环境里还没有安装这个可选包。
