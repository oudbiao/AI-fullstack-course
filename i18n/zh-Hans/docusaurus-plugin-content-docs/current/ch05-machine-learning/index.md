...
:::info 实操检查点
如果你想在选择大项目之前先完整跟跑一次，可以从 [实操工作坊：构建可复现的 ML 证据包](./ch06-projects/05-hands-on-ml-workshop.md) 开始。它会生成本地数据集、baseline、Pipeline、模型对比、阈值复盘、错误样本、泄漏记录和 README 证据。
:::
...
如果你想先做一次按步骤跟跑的项目彩排，可以在房价、客户流失、用户分群或 Kaggle 项目前，先完成 [实操工作坊：构建可复现的 ML 证据包](./ch06-projects/05-hands-on-ml-workshop.md)。
