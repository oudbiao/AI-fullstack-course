...
如果你想按历史进程理解每次技术突破，可以先读 [5.1.2 机器学习历史突破主线](./ch01-ml-basics/04-history-breakthroughs.md)。它会把 Bayes、MLE、EM、线性模型、决策树、SVM、随机森林、Boosting、XGBoost 和 sklearn 分配到本章对应小节，让你知道每个算法为什么会出现。
...
:::info 实操检查点
如果你想在选择大项目之前先完整跟跑一次，可以从 [5.6.6 实操工作坊：构建可复现的 ML 证据包](./ch06-projects/05-hands-on-ml-workshop.md) 开始。它会生成本地数据集、baseline、Pipeline、模型对比、阈值复盘、错误样本、泄漏记录和 README 证据。
:::

## 本阶段学习路径

5.1 节学习机器学习基础概念，包括历史突破主线、任务类型、训练集和测试集、Scikit-learn 基本用法，以及数学如何流入机器学习。

5.2 节学习监督学习，包括线性回归、逻辑回归、决策树和集成学习。你会理解分类和回归任务如何建模。

5.3 节学习无监督学习，包括聚类、降维和异常检测。你会理解没有标签时如何发现结构。

5.4 节学习模型评估与选择，包括指标、交叉验证、偏差方差和调参。这里决定你是否能判断模型到底好不好。

5.5 节学习特征工程。对很多真实表格数据项目来说，特征理解和处理比模型选择更重要。

5.6 节进入实战项目，把完整建模流程跑通。
...
如果你想先做一次按步骤跟跑的项目彩排，可以在房价、客户流失、用户分群或 Kaggle 项目前，先完成 [5.6.6 实操工作坊：构建可复现的 ML 证据包](./ch06-projects/05-hands-on-ml-workshop.md)。

如果你想看更细的学习顺序，可以阅读 [5.0 学习指南：机器学习怎么学最不容易学乱](./study-guide.md)。
