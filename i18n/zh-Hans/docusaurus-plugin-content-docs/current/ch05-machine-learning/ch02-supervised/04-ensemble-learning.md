...
![GBDT 残差纠错漫画](/img/course/ch05-ensemble-gbdt-residual-correction.png)

先看图，再看公式。第 1 棵树先给出一个粗略答案，然后模型会追问：“我还错在哪里？”这些还没预测好的部分就是**残差**。下一棵树不会从头再做一遍任务，而是学习一个小修正。很多轮之后，整个模型就变成了“许多小修正的总和”。
...
- **GBDT** 是 **Gradient Boosting Decision Tree**，意思是梯度提升决策树。它是一种 Boosting 方法，弱学习器通常是浅层决策树。
- **残差** 是当前模型还没预测好的部分。在回归里，一个简单残差就是 `真实值 - 当前预测`。
...
不要把这张对比表理解成“某个模型永远最好”。更安全的读法是：

- **XGBoost** 适合做一个稳定、强力的 Boosting 默认选择。
...
![Stacking 防泄漏流程漫画](/img/course/ch05-ensemble-stacking-leakage-safe.png)

Stacking 最容易出问题的地方是**数据泄漏**。如果基础模型在训练过的同一批样本上再预测，这些预测可能会虚高，因为模型已经“见过答案”。`StackingClassifier(cv=5)` 会用交叉验证生成**折外预测**：每一行训练数据的预测，都来自没有训练过这一行的基础模型。

先把几个词讲清楚：

- **CV** 是 **cross-validation**，也就是交叉验证。它把数据分成几折，用一部分训练，用另一部分验证。
- **折外预测** 是指某一行数据的预测来自“没有训练过这一行”的模型。
- **元学习器** 是最后的组合模型，负责学习如何把基础模型的预测合成最终预测。
- **泄漏** 是答案信息或未来信息偷偷进入训练过程，让评估结果看起来比真实上线效果更好。
