...
![学习率 sweep 输出结果图](/img/course/ch06-lr-sweep-result-map.webp)
...
3. 增加 seed 后要比较均值和波动，避免被一次好运或坏运误导。
4. 下一步决策应具体，例如“在 `0.03` 到 `0.3` 之间做更细搜索，并保留两个 seed”。
5. 单变量实验能把结果变化归因到一个因素，调参记录更清楚，也更容易复现。
