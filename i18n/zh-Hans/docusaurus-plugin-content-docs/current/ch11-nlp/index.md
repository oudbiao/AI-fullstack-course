...
11.1 节学习文本处理基础，包括 NLP 地图、文本预处理和文本表示。

11.2 节学习词嵌入与语言模型，理解词向量、上下文表示和语言模型的基本思想。

11.3 节学习文本分类，这是很多业务文本任务的入门项目。

11.4 节学习序列标注，包括命名实体识别和 BiLSTM-CRF 等方法。

11.5 节学习 Seq2Seq 与注意力，理解机器翻译和生成任务的重要历史路线。

11.6 节学习预训练语言模型，包括 BERT、GPT、T5 和 Transformers 库。
...
如果你想在选择大项目之前先跑一个 baseline，请先完成 [11.7.6 实操：构建一个可复现的 NLP 迷你流水线](./ch07-projects/05-hands-on-nlp-workshop.md)。它会用本地脚本生成数据文件、TF-IDF 分类、检索问答、抽取式摘要、信息抽取结果、指标和失败报告。

如果你想看更细的学习节奏，可以阅读 [11.0 学习指南：自然语言处理怎么学最不容易学乱](./study-guide.md)。
