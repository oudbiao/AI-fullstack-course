...
用这些任务把 NLP 落到真实文本工作里。即使有强大的 LLM，你仍然需要清楚的标签、来源证据、指标和失败样本。

| 任务 | 产出物 | 通过标准 |
|---|---|---|
| 理解文本预处理 | 一个文本清洗脚本 | 能处理分词、大小写、停用词、标点和特殊字符 |
| 完成文本表示实验 | 一份表示方法对比记录 | 能比较 BoW、TF-IDF、Embedding 和预训练模型表示 |
| 完成文本分类任务 | 一个分类 Demo | 能解释标签、数据划分、指标和错误样本 |
| 完成抽取或摘要练习 | 一个信息抽取或摘要示例 | 能解释字段边界、事实一致性和评估方法 |
| 跑通可复现 NLP 迷你流水线 | `nlp_workshop_run/` 证据文件夹 | 能解释 outputs、reports、metrics 和 `reports/failure_cases.md` |
| 完成一个阶段项目 | 一个小型文本理解项目 | 有输入输出、指标、失败样本和 README |
...
在选择更大的项目之前，建议先跑一遍 [11.7.6 可复现 NLP 迷你流水线](./ch07-projects/05-hands-on-nlp-workshop.md)。它是本阶段 baseline：一个脚本、本地文本数据、tokenization、TF-IDF、分类、检索问答、摘要、抽取、指标和失败样本。
...
![NLP 文本到产物流水线图](/img/course/ch11-workshop-text-to-artifacts-pipeline-map.png)

如果你想把本阶段成果沉淀到作品集，建议至少保留下面这些文件或等价材料。

| 交付物 | 说明 |
|---|---|
| `text_cleaning.py` | 文本清洗、分词、归一化和样例输出 |
| `label_guide.md` | 标签定义、边界案例、正反例和标注规则 |
| `classification_report.md` | 指标、混淆矩阵、错误样本和模型对比 |
| `extraction_examples.jsonl` | 信息抽取或结构化输出示例 |
...
- 文本清洗为什么会影响模型表现？
- TF-IDF 和 Embedding 有什么区别？
- 为什么文本分类需要标签规则？
- 摘要和抽取如何检查事实一致性？
- 什么时候传统 NLP 方法比 LLM 更合适？
