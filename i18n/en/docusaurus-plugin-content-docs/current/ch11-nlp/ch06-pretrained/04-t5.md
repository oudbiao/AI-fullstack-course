...
![T5 text-to-text task unification diagram](/img/course/ch11-t5-text-to-text-task-unification-map-en.png)
...
## 1. What is the most important idea in T5?

### 1.1 Rewrite different tasks as text-to-text
...
## 2. What is the difference between T5 and BERT / GPT?

### 2.1 BERT is more like a representation learning backbone
...
### 2.2 GPT is more like an autoregressive generator
...
### 2.3 T5 emphasizes a unified task interface
...
## 3. Run a minimal text-to-text example first

```python
tasks = [
    {"input": "translate English to Chinese: hello world", "target": "hello world"},
...
### 3.2 What is the biggest difference from a traditional classification interface?
...
### 3.3 What should beginners remember first when learning T5?
...
### 3.4 Another minimal example of "same interface, different tasks"

```python
examples = [
    ("translate English to Chinese: good morning", "good morning"),
...
## 4. Why does T5 feel so natural for many tasks?

### 4.1 Because many NLP tasks can already be viewed as text transformation
...
### 4.4 The safest default order when rewriting a task as text-to-text for the first time
...
### 5.1 Mistake 1: T5 is just another seq2seq model
...
### 5.2 Mistake 2: text-to-text is always better than other paradigms

No.  
It is a unified way of thinking, not an absolute guarantee of optimality for every task.

### 5.3 Mistake 3: A unified interface automatically means simplicity
