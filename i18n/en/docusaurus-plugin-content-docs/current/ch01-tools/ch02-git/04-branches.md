...
- From `<<<<<<< HEAD` to `=======` is the version from the **current branch** (main, containing Alice’s changes)
- From `=======` to `>>>>>>> bob/update-model` is the version from the **branch being merged in** (Bob’s)
